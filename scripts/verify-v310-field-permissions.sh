...
FIELD_PERMISSION_TESTS="${V310_FIELD_PERMISSION_TESTS:-FieldPermissionResolverTest,FieldAccessPermissionStepTest,FieldAccessPermissionIntegrationTest,SemanticServiceV3Test}"
...
verify_clean_reactor() {
  ...
  run_step "v3.10 clean reactor field-permission focused suite" \
    mvn -pl foggy-dataset-model -am clean test \
      -Dtest="$FIELD_PERMISSION_TESTS" \
...
verify_mysql8() {
  ...
  run_step "v3.10 MySQL8 field-permission focused suite" \
...
verify_postgres() {
  ...
  run_step "v3.10 PostgreSQL field-permission focused suite" \
    mvn -pl foggy-dataset-model -am test \
