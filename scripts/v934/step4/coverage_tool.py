...
def validate_contract_json(contract: dict[str, Any]) -> None:
    ...
    require(report_inventory == {
        ...
        "amendment_sha256": "263e2ecbfeda61555f56687789a6d0ad2a2eef3dff22f5fa06617c59d2eae241",
        ...
        "step4_changed_positive_reports": 5,
...
def validate_report_amendment(repo_root: Path, contract: dict[str, Any]) -> dict[str, int]:
    ...
    require(len(rows) == 9, "coverage report amendment: expected exact 9 rows")
    require(len({row["source_path"] for row in rows}) == 9, "coverage report amendment: duplicate source")
    require(len({row["report_fqcn"] for row in rows}) == 9, "coverage report amendment: duplicate report FQCN")
    counts: Counter[str] = Counter()
    testcase_delta = 0
    for number, row in enumerate(rows, 1):
        ...
        if change_kind == "new-positive-report":
            ...
            require(row["disposition"] == "step4-cardinality-amendment", f"coverage report amendment row {number}: invalid changed-report disposition")
    ...
    require(counts == Counter({"new-positive-report": 4, "changed-positive-report": 5}), "coverage report amendment: expected 4 new and 5 changed reports")
