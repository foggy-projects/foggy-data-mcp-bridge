...
AGGREGATE_MERGE_SEMANTICS = "exact-session-and-jacoco-class-id-probe-bitmap-union"
...
def load_ledger(step4_dir: Path, contract: dict[str, Any]) -> list[dict[str, str]]:
    jacoco_contract = contract.get("jacoco")
    if (
        not isinstance(jacoco_contract, dict)
        or jacoco_contract.get("class_id_consistency_scope")
        != EXPECTED_CLASS_ID_CONSISTENCY_SCOPE
    ):
        reject(
            "E_CONTRACT",
            "JaCoCo class-ID consistency scope must be the frozen production class universe",
        )
...
def inspect_exec(
    ...
    *,
    ...
    class_ids = [row["id"] for row in classes_data]
...
def require_consistent_production_class_ids(
    class_ids: dict[str, set[str]], workspace_classes: dict[str, dict[str, str]]
) -> None:
    mismatches = {
        name: {
            "expected": workspace_classes[name]["id"],
            "observed": sorted(ids),
        }
        for name, ids in class_ids.items()
        if name in workspace_classes and ids != {workspace_classes[name]["id"]}
    }
    if mismatches:
        preview = dict(list(sorted(mismatches.items()))[:5])
        reject(
            "E_CLASS_ID_MISMATCH",
            f"production class IDs differ from the frozen class universe: {preview}",
        )


def require_exact_execution_class_count(actual: int, expected: int) -> None:
    if actual != expected:
        reject(
            "E_AGGREGATE_CLASS_SET",
            "aggregate unique class-ID count differs from the verified exec manifest",
        )
...
def load_verified_exec_manifest(
    ...
    *,
    ...
    if manifest["class_id_consistency_scope"] != EXPECTED_CLASS_ID_CONSISTENCY_SCOPE:
        reject("E_MANIFEST", "exec manifest class-ID consistency scope differs")
    if type(manifest["unique_execution_classes"]) is not int or manifest["unique_execution_classes"] <= 0:
        reject("E_MANIFEST", "exec manifest unique class-ID count must be positive")
    for key, pattern in (
...
def verify_merged_execution_data(
    ...
    # JaCoCo execution identity is the CRC64 class ID. The same binary name may
    # legitimately occur with multiple IDs when lanes use different dependency
    # versions, classloaders, or generated bytecode. Preserve every ID while
    # requiring name/probe shape compatibility for repeated IDs.
    ...
    for inspected in inputs:
        ...
        for row in inspected["classes"]:
            current = expected_classes.get(row["id"])
            if current is None:
                expected_classes[row["id"]] = {
                    "name": row["name"],
                    "probe_count": row["probe_count"],
                    "probe_bitmap": bytearray(row["probe_bitmap"]),
                }
                continue
            if current["name"] != row["name"] or current["probe_count"] != row["probe_count"]:
                reject(
                    "E_AGGREGATE_CLASS_SHAPE",
                    f"input class shape differs for id={row['id']}",
                )
    ...
    actual_classes = {row["id"]: row for row in aggregate["classes"]}
    if len(actual_classes) != len(aggregate["classes"]):
        reject("E_AGGREGATE_CLASS_SHAPE", "aggregate contains duplicate class IDs")
    if set(actual_classes) != set(expected_classes):
        missing = [
            f"{expected_classes[class_id]['name']}@{class_id}"
            for class_id in sorted(set(expected_classes) - set(actual_classes))[:10]
        ]
        unexpected = [
            f"{actual_classes[class_id]['name']}@{class_id}"
            for class_id in sorted(set(actual_classes) - set(expected_classes))[:10]
        ]
        reject(
            "E_AGGREGATE_CLASS_SET",
    ...
    for class_id, expected in expected_classes.items():
        actual = actual_classes[class_id]
        if actual["name"] != expected["name"] or actual["probe_count"] != expected["probe_count"]:
            reject("E_AGGREGATE_CLASS_SHAPE", f"aggregate class shape differs for id={class_id}")
        if actual["probe_bitmap"] != bytes(expected["probe_bitmap"]):
            reject("E_AGGREGATE_PROBE_UNION", f"aggregate probe union differs for id={class_id}")
...
def verify_exec_set(args: argparse.Namespace) -> None:
    ...
    execution_class_ids: set[str] = set()
    ...
    for row in rows:
        ...
        for class_row in inspected["classes"]:
            ...
            execution_class_ids.add(class_row["id"])
    ...
    require_consistent_production_class_ids(class_ids, workspace_classes)
    class_universe_path = (
        repo_root
        / "target/v934-step4-coverage/runs"
        / args.run_id
    ...
    manifest = {
        ...
        **context_binding,
        ...
        "class_id_consistency_scope": EXPECTED_CLASS_ID_CONSISTENCY_SCOPE,
        "unique_execution_classes": len(execution_class_ids),
    ...
    print(
        ...
        f"sessions={total_sessions} classes={len(execution_class_ids)} "
...
def verify_aggregate(args: argparse.Namespace) -> None:
    ...
    require_exact_execution_class_count(
        totals["execution_class_count"], manifest["unique_execution_classes"]
    )
    result = {
        "schema_version": 1,
        ...
        "class_id_consistency_scope": EXPECTED_CLASS_ID_CONSISTENCY_SCOPE,
        "merge_semantics": AGGREGATE_MERGE_SEMANTICS,
        "status": "verified",
    }
...
def expect_success(action: Any) -> dict[str, str]:
    try:
        action()
    except CoverageError as exc:
        reject(
            "E_NEGATIVE_FALSE_RED",
            f"valid scoped execution-data fixture failed: {exc.code}: {exc.message}",
        )
    return {"expected_code": "PASS", "observed_code": "PASS", "status": "passed"}
...
def run_negatives(args: argparse.Namespace) -> None:
    ...
    workspace = {first_name: {"id": first_id, "sha256": "fixture", "module": "fixture"}}
    cases["production-class-id-match"] = expect_success(
        lambda: require_consistent_production_class_ids(
            {first_name: {first_id}}, workspace
        )
    )
    cases["production-class-id-mismatch"] = expect_failure(
        "E_CLASS_ID_MISMATCH",
        lambda: require_consistent_production_class_ids(
            {first_name: {first_id, forged_id}}, workspace
        ),
    )
    cases["production-class-id-only-forged"] = expect_failure(
        "E_CLASS_ID_MISMATCH",
        lambda: require_consistent_production_class_ids(
            {first_name: {forged_id}}, workspace
        ),
    )
    runtime_name = "org/example/RuntimeDependency"
    cases["runtime-same-name-multiple-ids"] = expect_success(
        lambda: require_consistent_production_class_ids(
            {runtime_name: {first_id, forged_id}}, workspace
        )
    )
    generated_name = "com/example/ProductionService$$SpringCGLIB$$0"
    generated_workspace = {
        generated_name: {"id": first_id, "sha256": "fixture", "module": "fixture"}
    }
    cases["generated-looking-production-mismatch"] = expect_failure(
        "E_CLASS_ID_MISMATCH",
        lambda: require_consistent_production_class_ids(
            {generated_name: {forged_id}}, generated_workspace
        ),
    )

    runtime_id_a = "1111111111111111"
    runtime_id_b = "2222222222222222"
    input_a = {
        "sessions": [{"id": "fixture-a", "start_ms": 1, "dump_ms": 2}],
        "classes": [
            {
                "id": runtime_id_a,
                "name": runtime_name,
    ...
    input_b = {
        "sessions": [{"id": "fixture-b", "start_ms": 3, "dump_ms": 4}],
        "classes": [
            {
                "id": runtime_id_a,
                "name": runtime_name,
    ...
    aggregate_sessions = input_a["sessions"] + input_b["sessions"]
    aggregate_classes = [
        {
            "id": runtime_id_a,
            "name": runtime_name,
    ...
    def verify_runtime_identity_union() -> None:
        totals = verify_merged_execution_data(
            [input_a, input_b],
            {"sessions": aggregate_sessions, "classes": aggregate_classes},
        )
        if totals != {
            "session_count": 2,
            "execution_class_count": 2,
            "covered_probe_count": 3,
        }:
            reject("E_NEGATIVE_ASSERT", f"unexpected aggregate totals: {totals}")

    cases["aggregate-runtime-same-name-multiple-ids"] = expect_success(
        verify_runtime_identity_union
    )
    cases["aggregate-manifest-class-id-count-drift"] = expect_failure(
        "E_AGGREGATE_CLASS_SET",
        lambda: require_exact_execution_class_count(2, 3),
    )
    cases["aggregate-missing-runtime-id"] = expect_failure(
        "E_AGGREGATE_CLASS_SET",
        lambda: verify_merged_execution_data(
            [input_a, input_b],
            {"sessions": aggregate_sessions, "classes": aggregate_classes[:1]},
        ),
    )
    shape_changed = [dict(row) for row in aggregate_classes]
    shape_changed[0]["name"] = "org/example/ChangedRuntimeDependency"
    cases["aggregate-id-shape-drift"] = expect_failure(
        "E_AGGREGATE_CLASS_SHAPE",
        lambda: verify_merged_execution_data(
            [input_a, input_b],
            {"sessions": aggregate_sessions, "classes": shape_changed},
        ),
    )
    input_shape_changed = {
        "sessions": input_b["sessions"],
        "classes": [dict(input_b["classes"][0])],
    }
    input_shape_changed["classes"][0]["probe_count"] = 5
    cases["aggregate-input-probe-shape-drift"] = expect_failure(
        "E_AGGREGATE_CLASS_SHAPE",
        lambda: verify_merged_execution_data(
            [input_a, input_shape_changed],
            {"sessions": aggregate_sessions, "classes": aggregate_classes},
        ),
    )
    union_changed = [dict(row) for row in aggregate_classes]
    union_changed[0]["probe_bitmap"] = b"\x01"
    union_changed[0]["covered_probes"] = 1
    cases["aggregate-probe-union-drift"] = expect_failure(
        "E_AGGREGATE_PROBE_UNION",
        lambda: verify_merged_execution_data(
            [input_a, input_b],
            {"sessions": aggregate_sessions, "classes": union_changed},
        ),
    )
    result = {
        "schema_version": 1,
