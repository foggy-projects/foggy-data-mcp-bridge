...
- Dynamic request-side `calculatedFields` that reference aggregate relation outputs are covered for source physical-column `deniedColumns`, including a transitive calculated-field chain; no implementation change was needed because `FieldAccessPermissionStep` already expands calculated-field dependencies before checking denied QM fields.
