...
| `foggy-dataset-model/src/main/java/com/foggyframework/dataset/db/model/semantic/service/impl/SemanticServiceV3Impl.java` | Exposes QM-only aggregate relation measure query columns in Java V3 JSON metadata. |
...
| `foggy-dataset-model/src/test/java/com/foggyframework/dataset/db/model/semantic/SemanticServiceV3Test.java` | Added JSON metadata regression for aggregate relation measure exposure. |
...
| Aggregate field metadata | pass-hardening | Aggregate relation fields inherit TM caption, resolved output type, formatter, AI/deprecation metadata, and expose `extData.aggregateRelation` lineage including aggregation/source/semantic scale/unit metadata. Java V3 JSON metadata also exposes QM-only aggregate relation measures with caption/type/aggregation/model attribution. |
...
- Aggregate relation runtime schema now carries business captions and type semantics from TM measures; Java V3 JSON metadata now exports QM-only aggregate relation measures. The remaining schema risk is query-cloud/data-viewer consumption and whether product metadata should surface full aggregate lineage, not the Java core schema export.
...
- Verify query-cloud/data-viewer consumes Java V3 JSON metadata for aggregate relation fields and decide whether full `extData.aggregateRelation` lineage should be surfaced in product metadata.
