...
| `foggy-dataset-model/src/test/java/com/foggyframework/dataset/db/model/semantic/SemanticServiceV3Test.java` | Added JSON metadata regression for aggregate relation measure and lineage exposure. |
...
| Aggregate field metadata | pass-hardening | Aggregate relation fields inherit TM caption, resolved output type, formatter, AI/deprecation metadata, and expose `extData.aggregateRelation` lineage including aggregation/source/semantic scale/unit metadata. Java V3 JSON metadata also exposes QM-only aggregate relation measures with caption/type/aggregation/model attribution and structured `aggregateRelation` lineage. |
