...
- Targeted SQLite execution tests, live MySQL 5.7 execution-plan evidence, PostgreSQL targeted aggregate-join evidence, PostgreSQL full prepared-service evidence, and normal join regression
...
| `foggy-dataset-model/src/main/java/com/foggyframework/dataset/db/model/impl/model/AggregateJoinTableModel.java` | Added synthetic right-side aggregate relation, SQL lowering, aggregate output metadata inheritance, same-physical-column left join-key expansion, and aggregate relation diagnostic attribution for explicit and relation-level aggregate joins. |
| `foggy-dataset-model/src/main/java/com/foggyframework/dataset/db/model/impl/model/AggregateRelationDiagnostic.java` | Extended planner diagnostics with `relationAlias`, `relationModel`, and `joinPath` while preserving pushed/retained/refused/projection-retained factory semantics. |
...
| `foggy-dataset-model/src/main/java/com/foggyframework/dataset/db/model/semantic/service/impl/SemanticServiceV3Impl.java` | Exposes QM-only aggregate relation measure query columns and structured aggregate relation lineage in Java V3 JSON metadata. |
...
| `foggy-dataset-model/src/test/java/com/foggyframework/dataset/db/model/semantic/SemanticServiceV3Test.java` | Added JSON metadata regression for aggregate relation measure and lineage exposure, including the stable seven-key lineage contract. |
...
| Aggregate relation diagnostics attribution | pass-hardening | Engine and Semantic V3 debug diagnostics now include the owning relation alias, RHS model, and join path, and targeted `orderBy` + `returnTotal` coverage proves the debug payload survives those query paths. |
| Aggregate field metadata | pass-hardening | Aggregate relation fields inherit TM caption, resolved output type, formatter, AI/deprecation metadata, and expose `extData.aggregateRelation` lineage including aggregation/source/semantic scale/unit metadata. Java V3 JSON metadata also exposes QM-only aggregate relation measures with caption/type/aggregation/model attribution and a stable structured `aggregateRelation` lineage contract. |
| Dialect/old database proof | pass-hardening-with-target-tms-risk | SQLite, live MySQL 5.7, PostgreSQL targeted aggregate-join, and PostgreSQL full prepared-service gates have passed, including the local TMS-style order+site composite-key fixture. SQL Server remains unclaimed and service-gated; the real target TMS database remains unavailable in this workspace. |
...
- The main performance gap is now narrower: MySQL 5.7 `EXPLAIN` confirms the RHS derived aggregate source can use keyed access (`uk_order_line`, `type=ref`, `rows=10`, `Using where`) after left join-key pushdown for the tested selective predicate, and the local TMS-style composite-key fixture executes on live MySQL 5.7. PostgreSQL targeted and full prepared-service evidence now pass. Real target TMS database plans remain follow-up evidence.
...
- Aggregate relation diagnostics are now attributable to the owning RHS relation through `relationAlias`, `relationModel`, and `joinPath`, so multi-relation AI/MCP debug consumers do not need to infer ownership from SQL aliases.
...
- Verify query-cloud/data-viewer consumes Java V3 JSON metadata for aggregate relation fields and decide how product metadata should display the exported `aggregateRelation` lineage.
