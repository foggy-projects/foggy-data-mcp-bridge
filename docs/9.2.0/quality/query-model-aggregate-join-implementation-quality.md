...
| Dialect/old database proof | pass-initial-with-risk | SQLite and live MySQL 5.7 execution passed, including the local TMS-style order+site composite-key fixture. PostgreSQL and the real target TMS database were not available in this environment. |
...
- Add PostgreSQL and real target TMS database SQL/explain evidence when those services are available.
