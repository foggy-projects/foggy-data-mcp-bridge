...
| Aggregate field metadata inheritance | accepted-with-risk | Core QueryModel schema and Java V3 JSON metadata expose inherited TM caption/type/aggregation and model attribution for aggregate relation measures. Query-cloud/data-viewer consumption and full lineage display remain separate chain checks. |
...
| `JAVA_HOME=/Users/fengjianguang/.jdk/temurin-17/Contents/Home mvn -pl foggy-dataset-model -am -P'!multi-db' -Dspring.profiles.active=sqlite -Dtest='SemanticServiceV3Test#testMetadata_Json_ShouldExposeAggregateRelationMeasure' -DfailIfNoTests=false -Dsurefire.failIfNoSpecifiedTests=false test` | success; Java V3 JSON metadata exposes aggregate relation output measure `salesAmount` with inherited caption/type/aggregation and QueryModel attribution; Tests run: 1, Failures: 0, Errors: 0, Skipped: 0. |
| `JAVA_HOME=/Users/fengjianguang/.jdk/temurin-17/Contents/Home mvn -pl foggy-dataset-model -am -P'!multi-db' -Dspring.profiles.active=sqlite -Dtest=SemanticServiceV3Test -DfailIfNoTests=false -Dsurefire.failIfNoSpecifiedTests=false test` | success; full V3 semantic metadata/query regression after aggregate relation JSON metadata exposure; Tests run: 22, Failures: 0, Errors: 0, Skipped: 0. |
...
- Query-cloud/data-viewer consumption of Java V3 JSON metadata for aggregate relation fields still needs upstream verification; full aggregate lineage display remains a product metadata decision.
