...
| Data Viewer background query loading indicator | implemented | `workitems/OPT-data-viewer-background-loading-indicator.md` | Frontend table refresh UX optimization: first load keeps delayed overlay, existing-data filter/sort/page refresh uses non-blocking toolbar status and top progress line. |
...
| Optional before-execute pipeline loop hook / PlanNormalizePipeline | plan-normalize-runtime-opt-in-local-verified | `workitems/OPT-loop-hook-pipeline-boundary.md` | `QueryExecutionStep` has a default-off bounded before-execute loop hook. `PlanNormalizePipeline` now exists as an explicit compose-plan normalizer with an empty `DerivedQueryPlan` wrapper rule and tests. `ComposeSqlCompiler`, `ComposeRelationCompiler`, and compose runtime/script requests can opt in, while all default paths remain unchanged. Main query pipeline remains one-way; after-execute remains single-pass; PreAgg migration and additional normalization rules are not enabled yet. |
...
- `workitems/OPT-data-viewer-background-loading-indicator.md` - Implemented Data Viewer frontend optimization for non-blocking background query loading indicators during filter, sort, page, refresh, and reload operations while preserving delayed overlay loading for initial or empty-data loads.
...
- `workitems/OPT-loop-hook-pipeline-boundary.md` - Optional before-execute pipeline loop hook design boundary, default-off `QueryExecutionStep` infrastructure, and first explicit `PlanNormalizePipeline` consumer; keeps the main query pipeline deterministic while allowing future bounded convergence-only consumers such as QueryPlan/CTE normalization.
