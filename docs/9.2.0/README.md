...
- Prepared MySQL dataset-model gate is recorded as passing; PostgreSQL and SQL Server evidence still depend on external prepared services in this local environment, despite existing `application-postgres.yml`, `application-sqlserver.yml`, and compose service definitions.
