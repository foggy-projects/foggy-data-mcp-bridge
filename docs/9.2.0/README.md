...
|---|---|---|---|
| QueryModel aggregate join | accepted-with-risks | `acceptance/query-model-aggregate-join-acceptance.md` | Complex predicate boundary now has SQLite and MySQL 5.7 evidence for mixed OR and AND `in`/range cases; local TMS-style order+site composite-key fixture is verified and registry promotion gate is defined. PostgreSQL/SQL Server profiles and compose services exist, but local services are unavailable in this shell; broader DB EXPLAIN, real target TMS package publication, and ETL promotion remain follow-up. |
