...
| Pivot outer cache Redis provider boundary | addon-fixture-local-verified | `workitems/PIVOT-outer-cache-redis-provider-boundary.md` | Core engine still has no Redis dependency. The optional `foggy-dataset-model-cache` addon now contains an opt-in `RedisPivotOuterCacheProvider` using `StringRedisTemplate`, with no-constructor-I/O, disabled-safe, copy-isolation, TTL, namespace/model eviction, invalid-payload, byte-estimation, and auto-config opt-in/backoff/no-Redis-operation tests. Live Redis startup/unavailable evidence remains the production promotion gate. |
...
- `workitems/PIVOT-outer-cache-redis-provider-boundary.md` - Redis/KV provider-module acceptance boundary; keeps Redis out of the core engine dependency graph, records the optional addon Redis provider fixture, and still requires live Redis startup/unavailable evidence before production promotion.
