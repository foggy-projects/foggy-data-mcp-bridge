...
| Optional before-execute pipeline loop hook / PlanNormalizePipeline | plan-normalize-pipeline-local-verified | `workitems/OPT-loop-hook-pipeline-boundary.md` | `QueryExecutionStep` has a default-off bounded before-execute loop hook. `PlanNormalizePipeline` now exists as an explicit compose-plan normalizer with an empty `DerivedQueryPlan` wrapper rule and tests. Main query pipeline remains one-way; after-execute remains single-pass; compiler default path and PreAgg migration are not enabled yet. |
