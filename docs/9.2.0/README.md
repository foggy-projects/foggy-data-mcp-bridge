...
| TMS target EXPLAIN and registry handoff | blocked-awaiting-authority-tms | `workitems/TMS-target-explain-and-registry-handoff-20260618.md` | #85 remains open, #96 now tracks `availablePieceCount` upstream confirmation, no local TMS/query-cloud-service checkout exists, and `foggy-model-registry` commit `881912a` keeps TMS package publication gated on real authority models and target database evidence. |
...
| QM predefined formula slice injection | local-verified-awaiting-upstream | `workitems/BUG-qm-predefined-formula-slice-injection.md` | Needs upstream issue #96 verification with `OrderStationStockProjectionQuery.availablePieceCount`. |
...
| Pivot outer cache design | e1b-provider-cross-process-local-verified-default-off | `workitems/PIVOT-outer-cache-design.md` | E1a telemetry remains the default behavior; E1b response cache can be explicitly enabled for successful flat/grid no-warning responses, with permission isolation, runtime bundle fingerprint, deployment bundle/freshness tokens, replaceable cache provider, default-safe external provider downgrade when Redis/KV is unavailable, no-Redis startup safety, provider-unavailable and no-false-store response diagnostics, distributed provider contract descriptor/key layout, distributed adapter/payload codec skeleton, reusable provider contract tests, invalidation event/result contracts, local replay-window guard for event-bus consumers, replay deduplication and source-node self-loop semantics, reusable invalidation broadcaster contract tests, simulated two-node fan-out semantics including partial-failure reporting, addon Redis invalidation publish/consume/listen primitives, local Docker Redis Pub/Sub invalidation evidence, live auto-configured listener context evidence, live cross-JVM Pub/Sub evidence, startup-safe listener auto-wiring, namespace/model cleanup hook, default-off admin eviction endpoint with failure payload diagnostics, bundle-removal cleanup with broadcaster-failure isolation, hit/store/store-skipped/TTL, eviction, deep-copy, and concurrent-hit diagnostics locally verified. |
| Pivot outer cache Redis provider boundary | redis-provider-cross-process-local-verified | `workitems/PIVOT-outer-cache-redis-provider-boundary.md` | Core engine still has no Redis dependency. The optional `foggy-dataset-model-cache` addon now contains an opt-in `RedisPivotOuterCacheProvider` using `StringRedisTemplate` plus Redis invalidation codec/broadcaster/consumer/listener primitives, with no-constructor-I/O, disabled-safe, copy-isolation, TTL, namespace/model eviction, invalid-payload, byte-estimation, auto-config opt-in/backoff/no-Redis-operation tests, invalid-endpoint startup safety, safe-provider downgrade/fail-fast evidence, local Docker Redis provider store/hit/eviction/TTL evidence, live Redis Pub/Sub source-to-remote-listener evidence, live auto-configured listener context evidence, live cross-JVM Pub/Sub evidence, listener disablement/startup-safe auto-wiring evidence, and local invalidation publish/consume/listen boundary tests. Production Redis deployment, subscription-loss recovery policy, and race-window evidence remain promotion gates. |
| Pivot dialect cascade evidence | sqlserver-promotion-gate-env-gated | `workitems/PIVOT-dialect-cascade-evidence-plan.md` | SQL Server cascade parity is blocked by live service availability and remains unclaimed; this gates only SQL Server-specific promotion. MySQL 5.7 remains large-domain transport supported and cascade refused. |
...
5. PIVOT-92-E1 E1b local response cache is operationally verified but remains default-off; runtime bundle fingerprint, provider SPI, default-safe provider-unavailable downgrade, reusable provider contract tests, distributed provider adapter/payload codec skeleton, invalidation broadcaster SPI, reusable fan-out contract tests, local replay-window guard, bundle-removal cleanup, default-off admin eviction endpoint, and addon Redis cross-JVM Pub/Sub evidence are now implemented. Promotion beyond local opt-in still needs a production distributed provider rollout or external durable fan-out implementation plus subscription-loss/race-window evidence.
...
- `workitems/PIVOT-outer-cache-design.md` - Staged outer Pivot cache design plus E1a default telemetry and E1b default-off cache evidence for permission isolation, runtime bundle/deployment freshness keying, replaceable cache provider, provider-unavailable safe downgrade for absent/unreachable Redis/KV by default, no-Redis startup safety, provider-unavailable/no-false-store response diagnostics, distributed provider contract descriptor/key layout, distributed adapter/payload codec skeleton, reusable provider contract tests, invalidation event/result contracts, local replay-window guard, replay deduplication and source-node self-loop semantics, invalidation broadcaster contract tests, simulated two-node fan-out semantics including partial-failure reporting, local Docker Redis Pub/Sub invalidation evidence, live auto-configured listener context evidence, live cross-JVM Pub/Sub evidence, namespace/model cleanup hook, default-off admin eviction endpoint with failure payload diagnostics, bundle-removal cleanup with broadcaster-failure isolation, lookup/miss/refused/hit/store/store-skipped/TTL-expiry diagnostics, eviction, deep-copy, and concurrent-hit safety.
- `workitems/PIVOT-outer-cache-redis-provider-boundary.md` - Redis/KV provider-module acceptance boundary; keeps Redis out of the core engine dependency graph, records the optional addon Redis provider fixture, local Docker Redis provider and Pub/Sub invalidation evidence, live auto-configured listener context evidence, live cross-JVM Pub/Sub evidence, invalid-endpoint startup safety, and unavailable-provider downgrade/fail-fast behavior; production Redis deployment, subscription-loss recovery policy, and race-window evidence remain promotion gates.
