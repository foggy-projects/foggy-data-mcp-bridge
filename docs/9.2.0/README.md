...
| Optional before-execute pipeline loop hook / PlanNormalizePipeline | plan-normalize-compile-opt-in-local-verified | `workitems/OPT-loop-hook-pipeline-boundary.md` | `QueryExecutionStep` has a default-off bounded before-execute loop hook. `PlanNormalizePipeline` now exists as an explicit compose-plan normalizer with an empty `DerivedQueryPlan` wrapper rule and tests. `ComposeSqlCompiler` can opt in through `CompileOptions.normalizePlan`, while the compiler default path remains unchanged. Main query pipeline remains one-way; after-execute remains single-pass; relation compiler default wiring and PreAgg migration are not enabled yet. |
