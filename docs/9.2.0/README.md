...
| Pivot outer cache design | e1b-operational-verified-default-off | `workitems/PIVOT-outer-cache-design.md` | E1a telemetry remains the default behavior; E1b process-local response cache can be explicitly enabled for successful flat/grid no-warning responses, with permission isolation, deployment bundle/freshness tokens in the key, namespace/model cleanup hook, hit/store/store-skipped/TTL, eviction, deep-copy, and concurrent-hit diagnostics locally verified. |
...
5. PIVOT-92-E1 E1b local response cache is operationally verified but remains default-off; promotion beyond local opt-in still needs automatic signed registry hash/fan-out integration, distributed provider, and cross-node coherency evidence.
...
- `workitems/PIVOT-outer-cache-design.md` - Staged outer Pivot cache design plus E1a default telemetry and E1b default-off local cache evidence for permission isolation, deployment bundle/freshness keying, namespace/model cleanup hook, lookup/miss/refused/hit/store/store-skipped/TTL-expiry diagnostics, eviction, deep-copy, and concurrent-hit safety.
