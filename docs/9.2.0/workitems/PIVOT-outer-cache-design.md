...
- Permission isolation, warning skip, maximum-size eviction, provider contract behavior, simulated two-node broadcaster fan-out semantics, replay deduplication key semantics, admin partial-failure diagnostics, deep-copy isolation, concurrent-hit behavior, runtime fingerprint provider, local broadcaster, admin endpoint delegation, and bundle-removal cleanup with broadcaster-failure isolation now have local tests, but no distributed cache provider, real event bus, or cross-process race evidence exists.
