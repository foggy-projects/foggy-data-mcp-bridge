...
- `PIVOT-outer-cache-redis-provider-boundary.md` now records the acceptance gates for a future Redis/KV provider module: no core Redis dependency, no constructor or lifecycle connection, disabled-state no-network behavior, operation-level safe downgrade, fail-fast mode, provider-contract reuse, distributed key/payload contract preservation, and response diagnostics evidence.
...
- A future Redis/KV provider module must also satisfy `PIVOT-outer-cache-redis-provider-boundary.md`; do not promote a provider that connects during bean construction, eager lifecycle callbacks, disabled state, or `isEnabled()`.
