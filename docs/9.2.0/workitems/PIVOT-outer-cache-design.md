...
- Permission isolation, warning skip, maximum-size eviction, provider contract behavior, provider-unavailable safe downgrade, no-false-store diagnostics, distributed provider descriptor/key layout, distributed adapter/payload codec skeleton, optional addon Redis provider fixture behavior, process-local replay window, simulated two-node broadcaster fan-out semantics, replay deduplication key semantics, source-node self-loop filtering simulation, admin partial-failure and broadcaster-exception diagnostics, deep-copy isolation, concurrent-hit behavior, runtime fingerprint provider, local broadcaster, admin endpoint delegation, and bundle-removal cleanup with broadcaster-failure isolation now have local tests, but no live Redis service test, real event bus, cross-process dedupe store/window, or cross-process race evidence exists.
