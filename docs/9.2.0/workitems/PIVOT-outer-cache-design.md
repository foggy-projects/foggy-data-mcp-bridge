...
- Permission isolation, warning skip, maximum-size eviction, provider contract behavior, deep-copy isolation, concurrent-hit behavior, runtime fingerprint provider, local broadcaster, admin endpoint delegation, and bundle-removal cleanup now have local tests, but no distributed cache provider or multi-node race evidence exists.
