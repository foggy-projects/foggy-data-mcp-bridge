...
- The optional cache addon now also contains `RedisPivotOuterCacheInvalidationCodec`, `RedisPivotOuterCacheInvalidationBroadcaster`, `RedisPivotOuterCacheInvalidationConsumer`, and `PivotOuterCacheRedisInvalidationAutoConfiguration`. This gives Redis Pub/Sub publish/consume primitives for the engine invalidation SPI, opt-in via `foggy.dataset.pivot.outer-cache.redis.invalidation-enabled=true`, with local evidence for event metadata, default namespace preservation, publish partial-failure reporting, replay dedupe, source-node self-loop filtering, invalid payload diagnostics, and auto-config construction without startup publish. It does not yet wire a mandatory listener container; host applications or the next engine slice must connect Redis messages to the consumer before claiming production cross-node coherency.
...
- `PIVOT-outer-cache-redis-provider-boundary.md` now records the acceptance gates and current addon fixture for Redis/KV provider modules: no core Redis dependency, no constructor or lifecycle connection, disabled-state no-network behavior, operation-level safe downgrade, fail-fast mode, provider-contract reuse, distributed key/payload contract preservation, response diagnostics evidence, local Docker Redis live evidence, and the remaining production/cross-process promotion gates.
...
- Optional Redis Pub/Sub invalidation can replace the default local broadcaster in the addon. The current addon provides a publisher and a reusable consumer with replay/self-loop protection, but production multi-node use still needs host listener wiring or a future auto-listener slice.
...
- If using the addon Redis invalidation broadcaster, make sure every node has a stable `node-id` and that a host-owned Redis listener forwards messages from the configured channel to `RedisPivotOuterCacheInvalidationConsumer.consume(...)`. Without that listener wiring, the addon publishes invalidation events but remote JVMs will not consume them automatically.
...
| `JAVA_HOME=/Users/fengjianguang/.jdk/temurin-17/Contents/Home mvn -pl addons/foggy-dataset-model-cache -am -Dtest='RedisPivotOuterCacheProviderTest,PivotOuterCacheRedisAutoConfigurationTest,RedisPivotOuterCacheUnavailableTest,RedisPivotOuterCacheProviderLiveTest,RedisPivotOuterCacheInvalidationBroadcasterTest,RedisPivotOuterCacheInvalidationConsumerTest,PivotOuterCacheRedisInvalidationAutoConfigurationTest' -DfailIfNoTests=false -Dsurefire.failIfNoSpecifiedTests=false test` | success; addon Redis provider fixture and Redis invalidation publish/consume primitives pass together, including no constructor publish, opt-in/backoff auto-config, event metadata/default namespace preservation, partial publish failure reporting, replay dedupe, source-node self-loop filtering, invalid payload diagnostics, and no live Redis requirement by default; Tests run: 30, Failures: 0, Errors: 0, Skipped: 3. |
...
| `JAVA_HOME=/Users/fengjianguang/.jdk/temurin-17/Contents/Home mvn -pl foggy-dataset-model,addons/foggy-dataset-model-cache -am -P'!multi-db' -Dspring.profiles.active=sqlite -Dtest='PivotOuterCacheStartupSafetyTest,PivotOuterCacheSafeProviderTest,PivotIntegrationTest#,PivotOuterCacheDistributedProviderAdapterTest,PivotOuterCacheDistributedProviderContractTest,PivotOuterCacheInvalidationReplayWindowTest,PivotOuterCacheInvalidationEventTest,PivotOuterCacheInvalidationFanOutContractTest,PivotOuterCacheAdminControllerTest,BundleLifecycleListenerTest,PivotOuterCacheOperationalSpiTest,PivotOuterResponseCacheTest,RedisPivotOuterCacheProviderTest,PivotOuterCacheRedisAutoConfigurationTest,RedisPivotOuterCacheUnavailableTest,RedisPivotOuterCacheProviderLiveTest,RedisPivotOuterCacheInvalidationBroadcasterTest,RedisPivotOuterCacheInvalidationConsumerTest,PivotOuterCacheRedisInvalidationAutoConfigurationTest' -DfailIfNoTests=false -Dsurefire.failIfNoSpecifiedTests=false test` | success; core outer-cache startup/safe-provider/store-failure/no-false-store/adapter/invalidation/admin/bundle lifecycle/operational SPI/local provider tests, addon Redis provider fixture, and Redis invalidation publish/consume primitives pass in one reactor run without requiring Redis by default; Core tests run: 62, Addon tests run: 30, Failures: 0, Errors: 0, Skipped: 3. |

## Remaining Boundary

- E1b remains default-off. The default provider and broadcaster are still process-local in core `foggy-dataset-model`; no Redis client, MQ publisher, or cross-node coherency implementation is added to the core dependency graph. The optional cache addon now has an opt-in Redis provider fixture plus Redis invalidation publish/consume primitives with invalid-endpoint startup safety, unavailable-provider downgrade/fail-fast evidence, local Docker Redis provider-operation evidence, and local invalidation event-boundary tests, but automatic listener wiring, production Redis deployment, and cross-process race evidence remain outside the accepted core engine contract.
...
- Permission isolation, warning skip, maximum-size eviction, provider contract behavior, provider-unavailable safe downgrade, no-false-store diagnostics, distributed provider descriptor/key layout, distributed adapter/payload codec skeleton, optional addon Redis provider fixture behavior, local Docker Redis provider operation behavior, Redis invalidation publish/consume boundary behavior, process-local replay window, simulated two-node broadcaster fan-out semantics, replay deduplication key semantics, source-node self-loop filtering simulation, admin partial-failure and broadcaster-exception diagnostics, deep-copy isolation, concurrent-hit behavior, runtime fingerprint provider, local broadcaster, admin endpoint delegation, and bundle-removal cleanup with broadcaster-failure isolation now have local tests, but no production Redis deployment, real event bus/listener wiring, cross-process dedupe store/window, or cross-process race evidence exists.
