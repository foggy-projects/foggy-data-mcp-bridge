...
status: e1b-provider-cross-process-local-verified-default-off
...
- Cache store emits `pivot.cache.store` only after the provider accepts the store operation. Provider unavailability during payload estimation, TTL lookup, or store emits `pivot.cache.provider_unavailable` plus `pivot.cache.store_skipped(reason=provider_unavailable)` and must not emit a false `store` event.
...
- The optional `addons/foggy-dataset-model-cache` module now contains `RedisPivotOuterCacheProvider`, `PivotOuterCacheStringStore`, `StringRedisTemplatePivotOuterCacheStore`, and `PivotOuterCacheRedisAutoConfiguration`. The provider maps the distributed adapter primitives to Redis string payloads and Redis sets for namespace/model indexes, uses Base64 for binary payload bytes, is opt-in via `foggy.dataset.pivot.outer-cache.redis.enabled=true`, does not add a Redis dependency or auto-configuration requirement to core `foggy-dataset-model`, and now has local Docker Redis evidence for live store/hit/evict/index/TTL/corrupt-payload behavior.
- The optional cache addon now also contains `RedisPivotOuterCacheInvalidationCodec`, `RedisPivotOuterCacheInvalidationBroadcaster`, `RedisPivotOuterCacheInvalidationConsumer`, `RedisPivotOuterCacheInvalidationMessageListener`, `RedisPivotOuterCacheInvalidationListenerLifecycle`, and `PivotOuterCacheRedisInvalidationAutoConfiguration`. This gives Redis Pub/Sub publish/consume/listen primitives for the engine invalidation SPI, opt-in via `foggy.dataset.pivot.outer-cache.redis.invalidation-enabled=true`, with local evidence for event metadata, default namespace preservation, publish partial-failure reporting, replay dedupe, source-node self-loop filtering, invalid payload diagnostics, auto-config construction without startup publish, listener-container backoff, listener disablement, invalid Redis endpoint startup safety, local Docker Redis Pub/Sub delivery from a source publisher to a remote listener, live Boot auto-config listener delivery across independent Spring contexts, and live cross-JVM delivery from an independent publisher process to an independent listener process. When a `RedisConnectionFactory` exists and `invalidation-listener-enabled=true`, the addon can auto-wire a dedicated listener container. The raw container is not Spring auto-started directly; a safe lifecycle controls startup through `invalidation-listener-auto-startup` and catches startup/stop failures so absent or unreachable Redis does not block context startup or Pivot execution. If listener startup fails, it remains non-running until the context/lifecycle is restarted or host supervision starts it after Redis recovers. Production cross-node coherency still needs Redis deployment, rollout evidence, and race/loss evidence under real model-refresh traffic.
...
- `PIVOT-outer-cache-redis-provider-boundary.md` now records the acceptance gates and current addon fixture for Redis/KV provider modules: no core Redis dependency, no constructor connection, disabled-state no-network behavior, operation-level safe downgrade, fail-fast mode, provider-contract reuse, distributed key/payload contract preservation, response diagnostics evidence, local Docker Redis provider, Pub/Sub invalidation, auto-config listener, and cross-JVM delivery evidence, startup-safe invalidation listener auto-wiring evidence, and the remaining production promotion gates.
...
- `addons/foggy-dataset-model-cache/src/main/java/com/foggyframework/dataset/db/model/cache/config/PivotOuterCacheRedisInvalidationAutoConfiguration.java`
- `addons/foggy-dataset-model-cache/src/main/java/com/foggyframework/dataset/db/model/cache/config/PivotOuterCacheRedisProperties.java`
- `addons/foggy-dataset-model-cache/src/main/java/com/foggyframework/dataset/db/model/cache/pivot/RedisPivotOuterCacheInvalidationBroadcaster.java`
- `addons/foggy-dataset-model-cache/src/main/java/com/foggyframework/dataset/db/model/cache/pivot/RedisPivotOuterCacheInvalidationCodec.java`
- `addons/foggy-dataset-model-cache/src/main/java/com/foggyframework/dataset/db/model/cache/pivot/RedisPivotOuterCacheInvalidationConsumer.java`
- `addons/foggy-dataset-model-cache/src/main/java/com/foggyframework/dataset/db/model/cache/pivot/RedisPivotOuterCacheInvalidationListenerLifecycle.java`
- `addons/foggy-dataset-model-cache/src/main/java/com/foggyframework/dataset/db/model/cache/pivot/RedisPivotOuterCacheInvalidationMessageListener.java`
- `addons/foggy-dataset-model-cache/src/test/java/com/foggyframework/dataset/db/model/cache/config/PivotOuterCacheRedisInvalidationAutoConfigurationLiveTest.java`
- `addons/foggy-dataset-model-cache/src/test/java/com/foggyframework/dataset/db/model/cache/config/PivotOuterCacheRedisInvalidationAutoConfigurationTest.java`
- `addons/foggy-dataset-model-cache/src/test/java/com/foggyframework/dataset/db/model/cache/config/PivotOuterCacheRedisInvalidationCrossProcessLiveTest.java`
- `addons/foggy-dataset-model-cache/src/test/java/com/foggyframework/dataset/db/model/cache/config/PivotOuterCacheRedisInvalidationCrossProcessWorker.java`
- `addons/foggy-dataset-model-cache/src/test/java/com/foggyframework/dataset/db/model/cache/pivot/RedisPivotOuterCacheInvalidationBroadcasterTest.java`
- `addons/foggy-dataset-model-cache/src/test/java/com/foggyframework/dataset/db/model/cache/pivot/RedisPivotOuterCacheInvalidationConsumerTest.java`
- `addons/foggy-dataset-model-cache/src/test/java/com/foggyframework/dataset/db/model/cache/pivot/RedisPivotOuterCacheInvalidationLiveTest.java`
- `addons/foggy-dataset-model-cache/src/test/java/com/foggyframework/dataset/db/model/cache/pivot/RedisPivotOuterCacheInvalidationMessageListenerTest.java`
...
- Optional Redis Pub/Sub invalidation can replace the default local broadcaster in the addon. The current addon provides a publisher, a reusable consumer with replay/self-loop protection, a Redis message listener, and a safe listener lifecycle. Auto listener wiring is available when a `RedisConnectionFactory` exists and `invalidation-listener-enabled=true`; if Redis is absent or unreachable, the lifecycle logs a warning and leaves the listener non-running without blocking the engine. Local Docker Redis evidence now covers provider operations, manually wired Pub/Sub invalidation delivery to a remote listener while skipping source-node self-loop, Boot auto-configured listener delivery across independent Spring contexts, and cross-JVM delivery from an independent publisher process to an independent listener process. Production multi-node use still needs Redis deployment, rollout, and race/loss evidence under real model-refresh traffic.
  ...
  dataset:
    pivot:
      outer-cache:
        ...
        redis:
          enabled: ${FOGGY_PIVOT_OUTER_CACHE_REDIS_ENABLED:false}
          key-prefix: ${FOGGY_PIVOT_OUTER_CACHE_REDIS_KEY_PREFIX:foggy:pivot:outer}
          invalidation-enabled: ${FOGGY_PIVOT_OUTER_CACHE_REDIS_INVALIDATION_ENABLED:false}
          invalidation-channel: ${FOGGY_PIVOT_OUTER_CACHE_REDIS_INVALIDATION_CHANNEL:}
          invalidation-listener-enabled: ${FOGGY_PIVOT_OUTER_CACHE_REDIS_INVALIDATION_LISTENER_ENABLED:true}
          invalidation-listener-auto-startup: ${FOGGY_PIVOT_OUTER_CACHE_REDIS_INVALIDATION_LISTENER_AUTO_STARTUP:true}
          invalidation-listener-recovery-interval-millis: ${FOGGY_PIVOT_OUTER_CACHE_REDIS_INVALIDATION_LISTENER_RECOVERY_INTERVAL_MS:5000}
          node-id: ${FOGGY_PIVOT_OUTER_CACHE_REDIS_NODE_ID:}
          replay-window-millis: ${FOGGY_PIVOT_OUTER_CACHE_REDIS_REPLAY_WINDOW_MS:60000}
          replay-window-maximum-entries: ${FOGGY_PIVOT_OUTER_CACHE_REDIS_REPLAY_WINDOW_MAX:1024}
```
...
- Redis/KV provider modules must also satisfy `PIVOT-outer-cache-redis-provider-boundary.md`; do not promote a provider that connects during bean construction, eager lifecycle callbacks, disabled state, or `isEnabled()`.
...
- If using the addon Redis invalidation broadcaster, make sure every node has a stable `node-id`. With a Spring `RedisConnectionFactory` present, the addon can auto-wire a Redis listener that forwards messages from the configured channel to `RedisPivotOuterCacheInvalidationConsumer.consume(...)`; set `invalidation-listener-enabled=false` when a host application owns that listener. The safe listener lifecycle is controlled by `invalidation-listener-auto-startup`; absent or unreachable Redis logs a warning and leaves the listener non-running, while the engine still starts and queries through the normal no-cache or safe-cache path.
- Treat addon Redis Pub/Sub invalidation as at-most-once delivery. `PivotOuterCacheInvalidationReplayWindow` is process-local dedupe/self-loop protection for messages already delivered to the JVM; it does not replay missed messages after disconnects, restarts, or subscription gaps. Use a durable event bus, freshness-token bump, short TTL, or manual/admin eviction if missed invalidations are unacceptable.
...
Default-off E1a emits only `lookup` plus either `miss` or `refused`. When E1b is explicitly enabled, flat/grid cacheable responses can also emit `hit`, `store`, `store_skipped`, and `evicted`. `store` means the provider accepted the write; unavailable providers must be counted through `provider_unavailable` and `store_skipped`, not as successful stores.
...
| `JAVA_HOME=/Users/fengjianguang/.jdk/temurin-17/Contents/Home mvn -pl addons/foggy-dataset-model-cache -am -Dtest='RedisPivotOuterCacheProviderTest,PivotOuterCacheRedisAutoConfigurationTest,RedisPivotOuterCacheUnavailableTest,RedisPivotOuterCacheProviderLiveTest,RedisPivotOuterCacheInvalidationBroadcasterTest,RedisPivotOuterCacheInvalidationConsumerTest,RedisPivotOuterCacheInvalidationMessageListenerTest,RedisPivotOuterCacheInvalidationLiveTest,PivotOuterCacheRedisInvalidationAutoConfigurationTest,PivotOuterCacheRedisInvalidationAutoConfigurationLiveTest,PivotOuterCacheRedisInvalidationCrossProcessLiveTest' -DfailIfNoTests=false -Dsurefire.failIfNoSpecifiedTests=false test` | success; addon Redis provider fixture and Redis invalidation publish/consume/listen primitives pass together, including no constructor publish, opt-in/backoff auto-config, listener disablement, listener container no-startup-publish/no-connection construction, invalid Redis endpoint startup safety, event metadata/default namespace preservation, partial publish failure reporting, replay dedupe, source-node self-loop filtering, invalid payload diagnostics, default-skipped live Pub/Sub coverage, default-skipped live auto-config listener coverage, default-skipped cross-process Pub/Sub coverage, and no live Redis requirement by default; Tests run: 38, Failures: 0, Errors: 0, Skipped: 6. |
| `JAVA_HOME=/Users/fengjianguang/.jdk/temurin-17/Contents/Home mvn -pl addons/foggy-dataset-model-cache -am -Dtest='RedisPivotOuterCacheProviderLiveTest,RedisPivotOuterCacheInvalidationLiveTest,PivotOuterCacheRedisInvalidationAutoConfigurationLiveTest,PivotOuterCacheRedisInvalidationCrossProcessLiveTest' -Dfoggy.redis.live.enabled=true -Dfoggy.redis.live.host=127.0.0.1 -Dfoggy.redis.live.port=16379 -DfailIfNoTests=false -Dsurefire.failIfNoSpecifiedTests=false test` | success against local Docker `redis:7-alpine` on `127.0.0.1:16379`; live Redis provider stores, hits from another provider instance, preserves copy isolation, evicts by namespace/model and all-scope indexes, expires TTL entries, removes corrupt payloads as misses, live Redis Pub/Sub invalidation delivers a namespace/model eviction event from a source node to a remote listener while the source-node self-loop is skipped, Boot auto-config wires two independent Spring contexts that publish and consume over the same Redis channel, and an independent publisher JVM reaches an independent listener JVM over the same channel; Tests run: 6, Failures: 0, Errors: 0, Skipped: 0. |
| `JAVA_HOME=/Users/fengjianguang/.jdk/temurin-17/Contents/Home mvn -pl foggy-dataset-model,addons/foggy-dataset-model-cache -am -P'!multi-db' -Dspring.profiles.active=sqlite -Dtest='PivotOuterCacheStartupSafetyTest,PivotOuterCacheSafeProviderTest,PivotIntegrationTest#,PivotOuterCacheDistributedProviderAdapterTest,PivotOuterCacheDistributedProviderContractTest,PivotOuterCacheInvalidationReplayWindowTest,PivotOuterCacheInvalidationEventTest,PivotOuterCacheInvalidationFanOutContractTest,PivotOuterCacheAdminControllerTest,BundleLifecycleListenerTest,PivotOuterCacheOperationalSpiTest,PivotOuterResponseCacheTest,RedisPivotOuterCacheProviderTest,PivotOuterCacheRedisAutoConfigurationTest,RedisPivotOuterCacheUnavailableTest,RedisPivotOuterCacheProviderLiveTest,RedisPivotOuterCacheInvalidationBroadcasterTest,RedisPivotOuterCacheInvalidationConsumerTest,RedisPivotOuterCacheInvalidationMessageListenerTest,PivotOuterCacheRedisInvalidationAutoConfigurationTest' -DfailIfNoTests=false -Dsurefire.failIfNoSpecifiedTests=false test` | success; core outer-cache startup/safe-provider/store-failure/no-false-store/adapter/invalidation/admin/bundle lifecycle/operational SPI/local provider tests, addon Redis provider fixture, and Redis invalidation publish/consume/listen primitives pass in one reactor run without requiring Redis by default; Core tests run: 62, Addon tests run: 35, Failures: 0, Errors: 0, Skipped: 3. |

## Remaining Boundary

- E1b remains default-off. The default provider and broadcaster are still process-local in core `foggy-dataset-model`; no Redis client, MQ publisher, or cross-node coherency implementation is added to the core dependency graph. The optional cache addon now has an opt-in Redis provider fixture plus Redis invalidation publish/consume/listen primitives with invalid-endpoint startup safety, unavailable-provider downgrade/fail-fast evidence, local Docker Redis provider-operation evidence, local Docker Redis Pub/Sub invalidation evidence, live auto-configured listener context evidence, local cross-JVM Pub/Sub evidence, startup-safe listener auto-wiring evidence, and local invalidation event-boundary tests, but production Redis deployment, subscription-loss recovery, and race-window evidence remain outside the accepted core engine contract.
...
- Permission isolation, warning skip, maximum-size eviction, provider contract behavior, provider-unavailable safe downgrade, no-false-store diagnostics, distributed provider descriptor/key layout, distributed adapter/payload codec skeleton, optional addon Redis provider fixture behavior, local Docker Redis provider operation behavior, Redis invalidation publish/consume/listen boundary behavior, local Docker Redis Pub/Sub invalidation behavior, live auto-configured listener context behavior, live cross-JVM Pub/Sub delivery behavior, startup-safe listener auto-wiring, process-local replay window, simulated two-node broadcaster fan-out semantics, replay deduplication key semantics, source-node self-loop filtering simulation, admin partial-failure and broadcaster-exception diagnostics, deep-copy isolation, concurrent-hit behavior, runtime fingerprint provider, local broadcaster, admin endpoint delegation, and bundle-removal cleanup with broadcaster-failure isolation now have local tests, but no production Redis deployment, production listener rollout evidence, real event bus rollout, durable replay/dedupe store, subscription-loss recovery evidence, or production race evidence exists.
