...
- Provider unavailability is now visible in response diagnostics through `pivot.cache.provider_unavailable` and cache miss/store-skip reason `provider_unavailable`, so Redis/KV outages are observable without making the query engine unavailable.
- Enabling `foggy.dataset.pivot.outer-cache.enabled=true` does not create a Redis dependency by itself. If no external `PivotOuterCacheProvider` bean is present, the engine uses the local provider path and startup/eviction remain available without Redis.
...
- `foggy-dataset-model/src/test/java/com/foggyframework/dataset/db/model/engine/pivot/PivotOuterCacheStartupSafetyTest.java`
...
| `pivot.cache.provider_unavailable` | `degraded` | model, keyHash, eligibilityStage, operation, providerName, reasonClass, reason, shapeClass |
...
When an optional external provider such as Redis/KV is unavailable and `fail-on-provider-unavailable=false`, the response should emit `pivot.cache.provider_unavailable`; the related miss or store-skip should use reason `provider_unavailable`.
...
- Outer-cache enabled startup succeeds without Redis or an external provider bean.
- Real Pivot execution with an unavailable external provider returns normal query results and emits provider-unavailable diagnostics.
