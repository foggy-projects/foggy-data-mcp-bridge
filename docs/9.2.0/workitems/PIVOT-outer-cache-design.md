...
- `PIVOT-outer-cache-redis-provider-boundary.md` now records the acceptance gates and current addon fixture for Redis/KV provider modules: no core Redis dependency, no constructor connection, disabled-state no-network behavior, operation-level safe downgrade, fail-fast mode, provider-contract reuse, distributed key/payload contract preservation, response diagnostics evidence, local Docker Redis provider and Pub/Sub invalidation evidence, startup-safe invalidation listener auto-wiring evidence, and the remaining production/cross-process promotion gates.
...
- Permission isolation, warning skip, maximum-size eviction, provider contract behavior, provider-unavailable safe downgrade, no-false-store diagnostics, distributed provider descriptor/key layout, distributed adapter/payload codec skeleton, optional addon Redis provider fixture behavior, local Docker Redis provider operation behavior, Redis invalidation publish/consume/listen boundary behavior, local Docker Redis Pub/Sub invalidation behavior, live auto-configured listener context behavior, startup-safe listener auto-wiring, process-local replay window, simulated two-node broadcaster fan-out semantics, replay deduplication key semantics, source-node self-loop filtering simulation, admin partial-failure and broadcaster-exception diagnostics, deep-copy isolation, concurrent-hit behavior, runtime fingerprint provider, local broadcaster, admin endpoint delegation, and bundle-removal cleanup with broadcaster-failure isolation now have local tests, but no production Redis deployment, production listener rollout evidence, real event bus rollout, cross-process dedupe store/window, or cross-process race evidence exists.
