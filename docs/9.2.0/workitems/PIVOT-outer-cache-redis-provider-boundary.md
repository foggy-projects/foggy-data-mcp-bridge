...
status: redis-provider-invalidation-local-verified
...
The optional `addons/foggy-dataset-model-cache` module now contains a first Redis provider fixture plus Redis Pub/Sub invalidation publish/consume primitives. This fixture is local-verified with mock/in-memory storage, unavailable Redis operation tests, invalidation codec/broadcaster/consumer tests, and a local Docker Redis live provider run. It is not yet a full production Redis or cross-process coherency signoff.
...
| Invalidation fan-out boundary | Optional Redis invalidation implementations preserve `PivotOuterCacheInvalidationEvent` scope semantics, default-namespace preservation, explicit-event replay dedupe, source-node self-loop filtering, partial publish failure reporting, and no constructor Redis I/O. |
...
- Invalidation primitives: `RedisPivotOuterCacheInvalidationCodec`, `RedisPivotOuterCacheInvalidationBroadcaster`, `RedisPivotOuterCacheInvalidationConsumer`, and `PivotOuterCacheRedisInvalidationAutoConfiguration`.
...
```bash
docker run -d --name foggy-pivot-redis-live -p 16379:6379 redis:7-alpine
...
The default addon test run includes `RedisPivotOuterCacheProviderLiveTest` but skips it unless `foggy.redis.live.enabled=true`.
