...
status: redis-provider-cross-process-local-verified
...
- Verified locally: constructor has no Redis interaction, disabled provider does not touch storage, `estimatePayloadBytes` does not touch storage, copy isolation holds, TTL expiry removes stale payloads, namespace/model eviction follows adapter indexes, invalid Base64 payloads are removed as misses, auto-config covers opt-in/backoff plus no Redis data-operation interaction during provider construction, invalid Redis endpoints do not block Spring context startup, unavailable Redis operations downgrade through `PivotOuterCacheSafeProvider` by default, explicit fail-fast rethrows, Redis invalidation publishing preserves event metadata and default namespace scope, publish failures are reported as partial invalidation results, consumer replay/self-loop/invalid-payload boundaries are covered, listener forwarding/invalid-payload handling is covered, listener auto-configuration is disable-able and startup-safe against an invalid Redis endpoint, a local Docker Redis run verifies live store/hit/evict/index/TTL/corrupt-payload behavior, a local Docker Redis Pub/Sub run verifies publisher-to-listener invalidation reaches a remote node while skipping the source-node self-loop, a local Docker Redis auto-config run verifies independent Spring contexts can publish and consume through the Boot-wired listener lifecycle, and a local Docker Redis cross-process run verifies an independent listener JVM receives an invalidation published by an independent publisher JVM.
- Still required for production promotion: production Redis deployment evidence, production listener rollout evidence, cross-process race-window evidence under load, Pub/Sub subscription-loss recovery evidence, and operational rollout evidence under real namespace/model registry refresh traffic.
...
The default addon test run includes `RedisPivotOuterCacheProviderLiveTest`, `RedisPivotOuterCacheInvalidationLiveTest`, `PivotOuterCacheRedisInvalidationAutoConfigurationLiveTest`, and `PivotOuterCacheRedisInvalidationCrossProcessLiveTest` but skips them unless `foggy.redis.live.enabled=true`.

`PivotOuterCacheRedisInvalidationCrossProcessLiveTest` starts a listener JVM and a publisher JVM with the current Surefire test classpath. The listener writes readiness and eviction evidence to a temporary directory, the publisher writes its local eviction/publish result, and the parent test verifies the remote JVM consumed the namespace/model invalidation through the shared Redis channel.
...
- Redis Pub/Sub is treated as an at-most-once invalidation transport in this addon. The process-local replay window only deduplicates explicit event ids already delivered to the JVM and filters source-node self-loop messages; it is not a durable replay log, distributed dedupe store, or recovery mechanism for messages missed while a node is disconnected or unsubscribed.
- If a deployment needs durable invalidation, it must provide a different `PivotOuterCacheInvalidationBroadcaster` backed by a durable event bus, bump `model-freshness-token` on model publish, or rely on short TTL plus manual/admin eviction as the fallback.
...
Minimal production opt-in for the addon should include:
  ...
  dataset:
    pivot:
      outer-cache:
        ...
        fail-on-provider-unavailable: false
        redis:
          enabled: true
          key-prefix: foggy:pivot:outer
          invalidation-enabled: true
          invalidation-channel: foggy:pivot:outer:invalidation
          invalidation-listener-enabled: true
          invalidation-listener-auto-startup: true
          node-id: ${HOSTNAME}
```

Production promotion still needs evidence that every service instance uses a stable unique `node-id`, the same channel/key-prefix/freshness policy, and the expected Redis connectivity/security settings from the host application.
