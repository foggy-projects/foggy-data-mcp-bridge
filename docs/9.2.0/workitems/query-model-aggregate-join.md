...
| Permission/system slice propagation | yes | covered-hardening: system slice lifecycle through QueryFacade is covered; structured accessBuilder field-ref join-key guard pushdown is covered; explicit tenant guard refs that resolve to an equivalent physical left join key are pushed into RHS aggregate `WHERE`; request-side `fieldAccess` allow/deny checks cover aggregate relation output fields; system-slice guard fields can filter without being exposed to user-visible columns. Raw SQL accessBuilder guards are covered as outer-only/no-pushdown boundary. |
