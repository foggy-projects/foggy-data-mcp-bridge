...
| Stage 4 Tests and Evidence | done-real-db-initial | Added SQLite execution parity, LEFT no-match, SQL-shape, fail-closed groupBy validation, relation-level default aggregation/projection pruning, query-time RHS pushdown SQL-shape, MySQL 5.7 real database execution, and ordinary multi-fact join regression. |
