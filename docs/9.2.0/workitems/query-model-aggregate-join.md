...
| Dialect fallback evidence | yes | done-initial: SQLite and live MySQL 5.7 profile passed; PostgreSQL and real target TMS database `EXPLAIN` remain follow-up because local Docker is unavailable and local PostgreSQL is closed. |
...
| TMS-style composite-key representative fixture | yes | done-local: `TmsStyleOrderStoreSalesAggregateRelationQueryModel` uses order + site grain and verifies request slices duplicate to RHS `agg_src.order_id = ?`, `agg_src.store_key = ?`, and `HAVING sum(agg_src.sales_amount) > ?` while native aggregate parity holds. Real TMS database/model-registry promotion remains follow-up. |
