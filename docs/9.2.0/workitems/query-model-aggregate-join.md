...
- SQLite targeted execution, local TMS-style order+site composite-key fixture execution, and MySQL 5.7 real database execution-plan evidence are recorded here. PostgreSQL and real target TMS database evidence remain follow-up items because local Docker is unavailable and the local PostgreSQL port is closed.
...
5. SQLite SQL-shape evidence, local TMS-style composite-key fixture evidence, and MySQL 5.7 real database `EXPLAIN` evidence: done-initial. PostgreSQL or real target TMS database `EXPLAIN` evidence remains follow-up.
...
| Dialect fallback evidence | yes | done-initial: SQLite and live MySQL 5.7 profile passed; PostgreSQL profile and compose service are present, but PostgreSQL `15432`/`5432` are closed and `docker`/`podman`/`psql` are unavailable in this shell. Real target TMS database `EXPLAIN` remains follow-up. |
...
| `command -v docker`, `command -v podman`, `command -v psql`, `command -v sqlcmd`, `command -v mysql`, `nc -z 127.0.0.1 15432`, `nc -z 127.0.0.1 5432`, `nc -z 127.0.0.1 11433`, `nc -z 127.0.0.1 1433`, `nc -z 127.0.0.1 13306` | Only `/usr/local/bin/mysql` exists. MySQL 5.7 port `13306` is reachable. PostgreSQL `15432`/`5432` and SQL Server `11433`/`1433` are closed. `docker`, `podman`, `psql`, and `sqlcmd` are unavailable in this shell. Profiles and compose service definitions exist for PostgreSQL `15432` and SQL Server `11433`, but no runnable local service was available. |
