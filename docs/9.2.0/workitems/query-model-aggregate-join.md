...
- Aggregate relation query objects now expose structured pushdown/projection diagnostics for planner evidence. Pushed filters record `decision=pushed`, target phase (`where` / `having`), source field, operator, and rendered RHS expression. Refused or retained filters and retained projection fallbacks record stable reason codes such as `OR_CONDITION_OUTER_ONLY`, `NULL_CHECK_OUTER_ONLY`, `UNSUPPORTED_OPERATOR`, `EMPTY_IN_VALUES`, `INVALID_RANGE_VALUE`, `NULL_VALUE_UNSUPPORTED`, `NO_AGGREGATE_EXPRESSION`, `NO_JOIN_KEY_MAPPING`, and `RAW_SQL_CONDITION_PROJECTION_PRUNING_DISABLED`. Semantic query responses surface these diagnostics under `debug.extra.aggregateRelationDiagnostics` when present, including the raw SQL projection-retained fallback, so SQL string assertions are not the only visibility into pushdown/projection decisions.
- The diagnostics JSON contract is now pinned by a dedicated unit test for the nine record keys `decision`, `reasonCode`, `field`, `op`, `target`, `expression`, `relationAlias`, `relationModel`, and `joinPath`, the pushed/retained/refused/projection-retained factory semantics, and the nine stable reason codes. Relation attribution makes AI/MCP debug output self-contained when a QM contains multiple aggregate relations. This prevents AI/MCP consumers from silently losing planner evidence after refactors.
...
set, relation attribution fields, and reason-code set directly, so downstream
consumers can treat these fields as a stable diagnostic contract.
...
| Stage 4 Tests and Evidence | done-real-db-initial | Added SQLite execution parity, LEFT no-match, SQL-shape, fail-closed groupBy validation, relation-level default aggregation/projection pruning including raw SQL projection-retained diagnostics, query-time RHS pushdown SQL-shape and OR outer-only boundary coverage, local TMS-style order+site composite-key fixture execution, MySQL 5.7 real database execution, and ordinary multi-fact join regression. |
...
| Aggregate relation diagnostics JSON contract | yes | done: `AggregateRelationDiagnosticContractTest` pins the nine JSON keys including `relationAlias`, `relationModel`, and `joinPath`, pushed/retained/refused/projection-retained factory semantics, and all nine stable reason codes; Semantic V3 debug coverage includes pushed, retained/refused, raw SQL projection-retained, and `orderBy` + `returnTotal` diagnostics. |
...
| Relation-level RHS projection pruning | yes | done: unreferenced aggregate relation measures are omitted from RHS SELECT; raw SQL accessBuilder conditions disable pruning, retain the full RHS projection, and expose `RAW_SQL_CONDITION_PROJECTION_PRUNING_DISABLED` as a retained projection diagnostic. |
...
| `JAVA_HOME=/Users/fengjianguang/.jdk/temurin-17/Contents/Home mvn -pl foggy-dataset-model -am -P'!multi-db' -Dspring.profiles.active=sqlite -Dtest='AggregateJoinQueryModelTest,AggregateRelationDiagnosticContractTest' -DfailIfNoTests=false -Dsurefire.failIfNoSpecifiedTests=false test` | success; full aggregate join SQLite regression plus diagnostics contract after raw SQL projection-retained diagnostic `RAW_SQL_CONDITION_PROJECTION_PRUNING_DISABLED`; Tests run: 56, Failures: 0, Errors: 0, Skipped: 0. |
| `JAVA_HOME=/Users/fengjianguang/.jdk/temurin-17/Contents/Home mvn -pl foggy-dataset-model -am -P'!multi-db' -Dspring.profiles.active=sqlite -Dtest='AggregateJoinQueryModelTest#semanticResponseShouldExposeAggregateRelationDiagnostics+semanticResponseShouldExposeRetainedAndRefusedAggregateRelationDiagnostics+semanticResponseShouldExposeRawSqlProjectionRetainedDiagnostics,AggregateRelationDiagnosticContractTest' -DfailIfNoTests=false -Dsurefire.failIfNoSpecifiedTests=false test` | success; Semantic V3 `debug.extra.aggregateRelationDiagnostics` exposes pushed diagnostics, retained/refused diagnostics, and raw SQL projection-retained diagnostic `RAW_SQL_CONDITION_PROJECTION_PRUNING_DISABLED`; diagnostics contract still pins the nine reason codes; Tests run: 6, Failures: 0, Errors: 0, Skipped: 0. |
| `JAVA_HOME=/Users/fengjianguang/.jdk/temurin-17/Contents/Home mvn -pl foggy-dataset-model -am -P'!multi-db' -Dspring.profiles.active=sqlite -Dtest='AggregateJoinQueryModelTest#,AggregateRelationDiagnosticContractTest' -DfailIfNoTests=false -Dsurefire.failIfNoSpecifiedTests=false test` | success; Semantic V3 diagnostics include relation attribution (`relationAlias` / `relationModel` / `joinPath`) for pushed, retained/refused, raw SQL projection-retained, and `orderBy` + `returnTotal` paths; Tests run: 7, Failures: 0, Errors: 0, Skipped: 0. |
| `JAVA_HOME=/Users/fengjianguang/.jdk/temurin-17/Contents/Home mvn -pl foggy-dataset-model -am -P'!multi-db' -Dspring.profiles.active=sqlite -Dtest='AggregateJoinQueryModelTest,AggregateRelationDiagnosticContractTest' -DfailIfNoTests=false -Dsurefire.failIfNoSpecifiedTests=false test` | success; full aggregate join SQLite regression plus diagnostics contract after relation attribution and Semantic V3 `orderBy` + `returnTotal` debug coverage; Tests run: 58, Failures: 0, Errors: 0, Skipped: 0. |
...
| `JAVA_HOME=/Users/fengjianguang/.jdk/temurin-17/Contents/Home mvn -pl foggy-dataset-model -am -P'!multi-db' -Dtest='AggregateRelationDiagnosticContractTest' -DfailIfNoTests=false -Dsurefire.failIfNoSpecifiedTests=false test` | success; aggregate relation diagnostics contract pins record component order, JSON keys, pushed/retained/refused factory semantics, and the original eight reason codes before the later raw SQL projection-retained diagnostic expansion; Tests run: 3, Failures: 0, Errors: 0, Skipped: 0. |
