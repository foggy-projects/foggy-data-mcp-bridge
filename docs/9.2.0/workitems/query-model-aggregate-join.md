...
- SQLite targeted execution, local TMS-style order+site composite-key fixture execution, MySQL 5.7 real database execution-plan evidence, and a PostgreSQL 15 targeted aggregate-join gate are recorded here. Full PostgreSQL module execution and real target TMS database evidence remain follow-up items.
...
5. SQLite SQL-shape evidence, local TMS-style composite-key fixture evidence, MySQL 5.7 real database `EXPLAIN` evidence, and PostgreSQL 15 targeted aggregate-join evidence: done-initial. Full PostgreSQL module execution and real target TMS database `EXPLAIN` evidence remain follow-up.
...
| Dialect fallback evidence | yes | done-initial: SQLite and live MySQL 5.7 profile passed; PostgreSQL targeted aggregate-join subset passed on local Docker `postgres:15-alpine`. Full PostgreSQL module execution, SQL Server evidence, and real target TMS database `EXPLAIN` remain follow-up. |
