...
| OR / complex condition groups | `A or B` | outer query only | Not pushed in this cut to avoid changing boolean semantics; join-key OR and aggregate-measure OR are covered as outer-only regressions. |
...
| `frontend-meta` / schema aggregate field exposure should be checked | core-engine-covered, upstream-follow-up | Core QueryColumn schema and Java V3 JSON metadata now expose aggregate relation measures with inherited caption/type/aggregation and model attribution. Query-cloud/data-viewer consumption and full lineage display remain separate metadata-chain follow-ups outside this Java engine cut. |
...
| Stage 4 Tests and Evidence | done-real-db-initial | Added SQLite execution parity, LEFT no-match, SQL-shape, fail-closed groupBy validation, relation-level default aggregation/projection pruning, query-time RHS pushdown SQL-shape and OR outer-only boundary coverage, MySQL 5.7 real database execution, and ordinary multi-fact join regression. |
| Query-Time RHS Filter Pushdown | done-hardening | Aggregate relation column metadata, measure `HAVING` duplication, group/source key `WHERE` duplication, left join-key key-domain pushdown, and structured accessBuilder field-ref guard pushdown are implemented for AND-only safe predicates. Left join-key matching now includes same-physical-column aliases across dimensions, properties, and measures, covering explicit tenant guard refs that use a no-table dimension `$id` while the aggregate relation join uses the property alias. |
...
| Aggregate Metadata Inheritance | done-hardening | Aggregate relation output columns inherit TM caption/type/formatter and expose source/aggregation/semantic lineage in `extData.aggregateRelation`; Java V3 JSON metadata also exposes QM-only aggregate relation measures such as `salesAmount` with caption/type/aggregation/model attribution. Query-cloud/data-viewer consumption remains upstream chain follow-up. |
...
| Query-time OR / complex predicate boundary | yes | done: OR join-key slices and OR aggregate-measure slices remain outer-query only and are not copied into RHS `WHERE` / `HAVING`. |
