...
- [ ] AC-1: Console shell 增加 desktop/mobile 可达 route/nav，直接 URL、login redirect、logout 与现有 navigation
  均不回归；页面清楚标记 Runtime-global，不误报为 Namespace 私有。
- [ ] AC-2: typed client 忠实解析 accepted inventory DTO；不记录/存储/拼接 absolute path、storeId、模型内容或凭据。
- [ ] AC-3: summary 显示 overall health、capturedAt、total objects/bytes、must retain/candidate/unknown/blocked，bytes 使用
  可读且精确 title；两个 root health/bytes/reasons 明确。
- [ ] AC-4: object ledger 可按 store/reference class/blocked/filter 搜索，显示 type/identity/status/bytes/references/reason；
  空值和长 identity 在 desktop/mobile 可读且可访问。
- [ ] AC-5: `NOT_INITIALIZED/PARTIAL/HEALTHY/BLOCKED`、transport error、auth failure、capability unsupported、loading/
  empty 均有差异化状态和可执行下一步；刷新具备 stale-response protection。
- [ ] AC-6: 页面明确 candidate 不是删除授权，unknown/blocked 必须保留；无 cleanup、repair 或路径操作 UI。
- [ ] AC-7: unit 覆盖 type/filter/format/stale behavior；desktop+mobile Playwright 覆盖 login→navigation→healthy/blocked→
  filter→refresh/error，并保存可复核 screenshot，console/page errors 为零。
- [ ] AC-8: `npm run typecheck`、unit、build、focused/affected Playwright、必要 Maven Console smoke 和完整 diff checks
