...
| Negative runtime boundary is enforced | accepted | Missing threshold, threshold unit ambiguity, unsupported SLA, physical SQL, prediction/personnel advice, unresponded formula ambiguity, conflicting time windows, and first-response/resolution field mismatch fail closed. |
...
| `mvn -pl foggy-dataset-mcp -am -Dtest=QueryExpertServiceRoutingCalibrationTest -Dsurefire.failIfNoSpecifiedTests=false test -P'!multi-db'` after expanded negative boundaries | passed: `Tests run: 26, Failures: 0, Errors: 0, Skipped: 0`. |
| `python3 experiments/spider-routing-eval/scripts/test_score_service_ticket_negative_gate.py` after expanded negative profiles | passed: configured negative profiles score `8/8`, residuals `0`. |
| `make -C experiments/spider-routing-eval gate-v39-semantic-promoted-offline-ci` after expanded negative boundaries | passed: order `6/6`, ServiceTicket suite `17/17`, residuals `0`. |
...
| SLA miss count and unresponded count are easy for LLMs to conflate. | Accepted with guardrail: explicit cutoff/reference-time unresponded predicates are required; ambiguous difference arithmetic fails validation/scoring and preflight clarification. |
