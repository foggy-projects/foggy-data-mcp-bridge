...
| `docs/9.1.0/detailed_design/16_service_ticket_sla_recipe_contract_index.md` | Added a single signed/unsigned recipe contract index for ServiceTicket SLA so mapper, schema, fixture, scorer, and runtime gates have one registry. |
...
| DSL_CTE pause/hold exclusion duration compile guard | passed on 2026-06-02: direct `net_hours_between(...)` / `pause_excluded_hours_between(...)` / `hold_excluded_hours_between(...)` / `customer_wait_excluded_hours_between(...)` formulas defer with explicit unsigned pause/hold exclusion duration reason instead of compiling through the natural-hour SLA bridge; `DslCteAcceptanceSampleTest` reached `165/165`. |
| Query model description consistency guard | passed on 2026-06-02: `QueryModelDescriptionConsistencyTest` verifies full/basic/no-vector schema descriptions all retain the ServiceTicket SLA signed recipe and unsigned formula boundary snippets. |
...
| Testing | complete | Focused launcher regression, package build, model/MCP regression, direct MCP DSL_CTE probe, targeted NL replay, priority-aware holdout, negative runtime gate, pause/hold exclusion boundary, business-hours SLA boundary, and combined stable gate suite completed. |
| Experience | N/A | Pure backend/model fixture change; no UI surface. |
| Semantic promotion | ready | Latest `biz-024`, SLA holdout, and negative ServiceTicket runtime gates are failure-free across the tracked runtime baselines; latest offline combined stable suite is `23/23` with residuals `0`. |
...
| Pause/hold compile guard | Pause/hold/customer-wait net-duration formulas are guarded at validation time; keep this unsigned until pause interval fields, customer-wait policy, overlap handling, and net-duration semantics are implemented. |
| Recipe contract index | `16_service_ticket_sla_recipe_contract_index.md` is now the first lookup point for adding or promoting a ServiceTicket SLA recipe; new recipes must update TM/QM fixtures, DSL_CTE mapper, Java tests, schema descriptions, runtime preflight, replay scorer, and version docs together. |
