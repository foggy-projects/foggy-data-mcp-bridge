...
| M3 | foggy-fsscript `ComposeQueryDialect`（`isKeywordAsIdentifier(FROM, '(')`）+ Layer A 宿主沙箱 | **`python-ready-for-review / java-pending`** | 2026-04-21 | — | Java 侧待开工 | Python `COMPOSE_QUERY_DIALECT` 落地 `foggy.fsscript.parser.dialect`（移除 `from` 保留字；`if / return / const` 等保留字不受影响）· `foggy.dataset_model.engine.compose.sandbox` 包含 14 个 sandbox-violation 错误码（Layer A 8 + B 3 + C 3）+ `ComposeSandboxViolationError`（构造期校验 code/phase，暴露 layer/kind/script_location）· 27 tests 全绿（12 dialect + 15 sandbox）· 全仓 **2591 passed / 1 skipped**（M2 基线 2564 + 27 = 2591，0 failed）· Layer A/B/C 验证器的静态 AST 扫描属于 M9 真正落地范围，本期仅交付契约 |
...
| M5 | BaseModelPlan 首次使用 hook + `authorityResolver.resolve` 链路 + 请求去重 | `ready-to-start` | — | — | — | F-3 Java 已修；两端 F-3 均解除，集成测试可靠性恢复 |
