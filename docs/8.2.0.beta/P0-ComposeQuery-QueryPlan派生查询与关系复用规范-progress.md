...
| M5 | BaseModelPlan 首次使用 hook + `authorityResolver.resolve` 链路 + 请求去重 | **`python-ready-for-review / java-pending`** | 2026-04-21 | — | Java 侧待开工 | Python `foggy.dataset_model.engine.compose.authority` 子包（`model_info.py` / `collector.py` / `resolver.py` / `apply.py`）· 公开 API 5 项：`ModelInfoProvider` Protocol + `NullModelInfoProvider` 降级 + `collect_base_models` 首次使用去重 + `resolve_authority_for_plan` 批量 resolve + `apply_field_access_to_schema` 白名单过滤 · `collect_base_models` 按 QM model name 首次出现去重（同一 QM 在 union/join 多次引用共享一条 binding）· `resolve_authority_for_plan` 一次 resolve 覆盖整棵 plan 树（请求级去重）· 五个 fail-closed 分支：`RESOLVER_NOT_AVAILABLE`（空 resolver）/ 非 `AuthorityResolutionError` 异常 → `UPSTREAM_FAILURE`（保留 `__cause__`）/ 返回非 `AuthorityResolution` → `INVALID_RESPONSE` / 返回 bindings 缺 key → `MODEL_BINDING_MISSING`（请求顺序决定性）/ 返回 bindings 多 key 或 value 非 `ModelBinding` → `INVALID_RESPONSE` · `apply_field_access_to_schema` 仅处理 `field_access` 白名单（`None` → no-op / `[]` → 空 schema / `[names]` → 保序过滤）· **不**动 `denied_columns`（物理列解析需要 v1.3 `PhysicalColumnMapping` → M6 SQL 编译器范围）· 4 份测试 **51 tests 全绿**（collector 10 + resolver 28 + apply 17 + public_api 3 · 去 hooking 方式构造的 non-binding 边界用例）· 全仓 **2709 passed / 1 skipped**（M4 基线 2658 + 51 = 2709，0 failed） |
| M6 | SQL 编译器：`query / union / join` 支持 + CTE / 子查询方言回退 | `not-started` | — | — | — | F-3 Java 判定在 v1.6 P0-BUG-F3 M6 已完成 |
