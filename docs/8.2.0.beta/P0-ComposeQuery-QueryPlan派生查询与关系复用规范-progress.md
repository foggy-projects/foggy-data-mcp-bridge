...
| M9 | 三层白名单防护测试集（Layer A/B/C 24 条用例，Java+Python 两仓同名落地） | **`ready-for-review`** | 2026-04-24 | 2026-04-25 | — | **Java 侧已落地**（Python 侧 M9 在 M3 中已完成一部分，剩余 AST 扫描在后续同步）。Java 侧新建 `com.foggyframework.dataset.db.model.engine.compose.sandbox` 下 3 个拦截器：`ScriptSourceScanner`（源码级正则，拦截 eval/Function/fetch/RAW_SQL/.raw() 等危险模式）、`ExpressionWhitelistValidator`（AST 转换后函数白名单与注入模式拦截）、`SecurityParamGuard`（防止业务参数覆盖鉴权参数）。24 个测试用例（A层10个，B层7个，C层7个）已 **100% PASS**。全仓回归 **1583 passed / 0 failures**。 |
| M10 | 集成测试 + 文档回写 + 签收 | `in-progress` | 2026-04-26 | — | — | 已补 Java 脚本资源真实 SQL parity：`ScriptResourceRealSqlParityTest` 3 tests 覆盖 derived/filter、join aggregate、union all，SQLite 与 MySQL 5.7 均 3 passed / 0 skipped；已补 `ComposedDataSetResultIntegrationTest` 1 test 覆盖 `withJoin` 延迟执行容器真实 DB/QM 执行、`execute()` 缓存与手写 SQL parity，SQLite 与 MySQL 5.7 均 1 passed / 0 skipped；已补 `ComposeScriptToolIntegrationTest` 2 tests 覆盖 `dataset.compose_script` Spring 工具注册、embedded-mode 真实脚本执行与手写 SQL parity；M11 收口计划见 `M11-CTE-TimeWindow-Coverage-Closure-Plan.md`；包含 `docs/8.2.0.beta/acceptance/` 签收记录；`columnsObj` follow-up：8.3.0.beta P2 已 accepted（见 `docs/8.3.0.beta/acceptance/P2-ComposeQuery-Columns-API-acceptance.md`），Java `columns` API 已收口为单字段 + 单 setter |
...
### 2026-04-26 (晚间)
- **Java 脚本资源真实 SQL parity 补强**：新增 `ScriptResourceRealSqlParityTest` 3 tests，将脚本资源从 preview-only 证据补到真实执行结果对比。
  - 新增 `real_sql_derived_query_scenario.js` / `real_sql_join_scenario.js` / `real_sql_union_scenario.js`，基于电商真实 QM（`FactSalesQueryModel` / `FactReturnQueryModel`）覆盖 derived/filter、join aggregate、union all。
  - 每个脚本通过 `ScriptRuntime.runScript(..., previewMode=false)` 自动执行 `plans`，并与手写 SQL 做逐行归一化比较。
  - 验证：SQLite `ScriptResourceRealSqlParityTest` 3 passed；SQLite `ScriptResourceRealSqlParityTest + ScriptRuntimeTest + ComposeRealSqlParityTest` 24 passed / 0 skipped；MySQL 5.7 `ScriptResourceRealSqlParityTest` 3 passed / 0 skipped。
- **ComposedDataSetResult 真实 DB/QM parity 补强**：新增 `ComposedDataSetResultIntegrationTest` 1 test，将 legacy `withJoin` 延迟执行容器从单纯对象行为补到真实执行证据。
  - 通过 `DslQueryFunction` 构造 `FactSalesQueryModel` 与 `FactReturnQueryModel` 两个真实 `DataSetResult`，再走 `withJoin("INNER", "product$id")` 生成 `ComposedDataSetResult`。
  - 断言 `execute()` 二次调用复用缓存对象，并将组合查询结果与手写 SQL 做逐行归一化比较。
  - 验证：SQLite `ComposedDataSetResultIntegrationTest` 1 passed；SQLite `ComposedDataSetResultIntegrationTest + DataSetResultTest + ScriptRuntimeTest + ComposeRealSqlParityTest + ScriptResourceRealSqlParityTest` 71 passed / 0 skipped；MySQL 5.7 `ComposedDataSetResultIntegrationTest` 1 passed / 0 skipped。
- **ComposeScriptTool MCP 端到端 parity 补强**：新增 `ComposeScriptToolIntegrationTest` 2 tests，把 MCP 工具层从 mock 单元测试补到 Spring integration + 真实 DB/QM 证据。
  - 测试配置新增 `composeAuthorityResolverFactory` 与同构 `ComposeScriptTool` bean，模拟 host embedded mode。
  - 用 `ComposeRuntimeBundle` 预置 `ComposeQueryContext`，通过 `dataset.compose_script` 执行 join 编排脚本，并与手写 SQL 做逐行归一化比较。
  - 验证：`mvn -pl foggy-dataset-mcp -am -Dtest=ComposeScriptToolIntegrationTest -DfailIfNoTests=false -Dsurefire.failIfNoSpecifiedTests=false -P!multi-db test` → 2 passed / 0 skipped。

### 2026-04-25 (凌晨)
- **M9 Java 侧三层沙箱防护落地** · `ready-for-review`：完成 `M9-三层沙箱防护测试脚手架.md` 中定义的全部防御。
  - 新建 `ScriptSourceScanner`：预编译期正则扫描，拦截 eval、Function、fetch、require 等 JS 层危险调用，以及 RAW_SQL 等越级 SQL 函数，和 `.raw()`、`.items.` 等越权 API 遍历。
  - 新建 `SecurityParamGuard`：防参数注入，阻止脚本在 `slice` 等结构里塞入 `authorization` / `tenantId` 等系统安全参数。
  - 新建 `ExpressionWhitelistValidator`：执行期白名单校验，允许 `SUM/DATE_DIFF` 等，阻止 `CHAR/SLEEP/LOAD_FILE`，并在 slice 值里探测 `UNION SELECT` 等 SQL 注入。
  - `ScriptRuntime` 集成：挂载三个拦截器，并注入 `params` 只读通道。修复了 `ComposeQueryDialect` 实际未被 fsscript 激活导致 `from(...)` 仍报保留字的 Bug。
  - **100% 绿灯**：24 个沙箱防逃逸测试（Layer A 10个，Layer B 7个，Layer C 7个）全部验证通过。全仓回归 **1583 passed / 0 failures**。

### 2026-04-24 (晚间)
- **Java M7 提示词 r2 · 吸收 plan-evaluator 复核**（3 blocker + 7 非阻塞项）：
  - **C1 · 工具命名冲突**：核实 `ComposeQueryTool(name="dataset.compose_query")` 已存在 · 新工具改名 `dataset.compose_script`（对齐存量 `dataset.*` 命名空间，不用 `compose.*`）；旧 `ComposeQueryTool` 仅在类级 Javadoc 加 `@implNote since 8.2.0.beta: use ComposeScriptTool for new compose queries; legacy 2-way-join scenarios only` 不加 `@Deprecated`（避免触发现有测试告警）；M7 scope 不改旧工具 body，M10 或 cleanup sprint 再议
  - **C2 · stale CLAUDE.md**：`foggy-dataset-mcp/CLAUDE.md` 描述的是另一个独立项目 `mcp-data-model-java`（package `com.foggy.mcp`、Spring AI 1.0.0-M4、端口 7108），与本仓 `com.foggyframework.dataset.mcp` 无关。frontmatter 的 `intended_for` 明写"直接忽略该份 CLAUDE.md，以现成代码（QueryModelTool / ComposeQueryTool）为参考"，避免子 agent 读错
  - **C3 · DataSource 解析**：实现路径锁死 —— `DataSourceQueryUtils.getDatasetTemplate(dataSource).getTemplate().queryForList(sql, params.toArray())`（与 `ComposedDataSetResult.execute():126` 同款）；`DataSource` 通过 `@Resource` 注入 `SemanticQueryServiceV3Impl`（参考 `ComposeQueryTool` 同款模式）；`routeModel` 本期占位不实装（javadoc 标 reserved）；单数据源部署下 Spring 自动 autowire 主 DataSource
  - S1 · §7.2 改为 Java-specific：核实 `DefaultExpEvaluator.newInstance()` **不做 builtin 预注入**（Python 有 17 个，Java 是空的）—— 区别在 `appCtx` 里的 Spring `@FsscriptExp` bean。M7 策略：`appCtx=null` 构造 evaluator · `ALLOWED_SCRIPT_GLOBALS = Set.of("from", "dsl")` 只 2 项，比 Python 的 19 项更严
  - S2 · §对齐原则 #8 登记：`QueryPlan.execute()` 返回类型 `Object → List<Map<String, Object>>` 是刻意 M2 API surface binary break，需在 progress.md 决策记录专门留痕（本条算登记）
  - S3 · §7.1 JWT 推迟：`foggy-auth` / `io.jsonwebtoken` 当前仓内**都不存在**（r2 grep 已核实）；本期**只实装嵌入模式**（host 通过 `ComposeRuntimeHolder.setBundle` 预置 `ComposeQueryContext`）；header 模式 stub 抛 `UnsupportedOperationException` 明写推迟到 M8+
  - S4 · foggy-dataset-mcp baseline 改为"开工时写死"
  - S5 · 预估 3.0 PD → **3.5 – 4.0 PD**（+0.1 Step 0 / +0.1 §7.4 / +0.2 buffer）
  - S6 · 流程图 DB 调用改成正文一致的 `DataSourceQueryUtils`
  - S7 · 停止条件增补 C1/C2/C3 meta-guard + `errorPayload / 成功分支` 切 `LinkedHashMap`（便于后续加 `warnings` 等字段扩展）
  - §附注 7 条非阻塞风险（多数据源 M8+ / header M8+ / ThreadLocal 泄漏防御 / factory 签名提醒 等）
...
### Execution Check-in · 2026-05-06 · Derived same-stage aggregate alias

- Background: Odoo AR-014 high-variance benchmark exposed a derived compose path where the LLM first used unsupported `plan.query({... having })`, then changed to `slice` while still filtering an aggregate alias created in the same derived stage (`count(invoiceDate$month) AS overdueMonths` filtered by `overdueMonths`). Python canonical fixed this as `DERIVED_QUERY_SAME_STAGE_ALIAS`; this check-in brings Java parity.
- Java change: `SchemaDerivation.derive()` now detects when a derived plan's `slice` references a current-stage explicit alias that is not present in the source output schema. It raises `compose-schema-error/derived-query/same-stage-alias` during `schema-derive` with guidance to add another `.query({ slice: [...] })` stage.
- Contract boundary: derived `having` remains intentionally unsupported. This change does not implement SQL HAVING on derived plans and does not rewrite `having` into `slice`.
- Tests: added coverage in `SchemaDerivationTest` for same-stage aggregate alias rejection, source-column slice allowance, and the correct two-stage pattern; updated `ComposeSchemaExceptionTest` to freeze the 17-code schema error catalog.
- Verification: `mvn -pl foggy-dataset-model "-Dtest=SchemaDerivationTest,ComposeSchemaExceptionTest" test` -> build success. Surefire ran the two test classes across default/mysql/postgres executions; each execution reported 46 tests, 0 failures.
