...
| M10 | 集成测试 + 文档回写 + 签收 | `in-progress` | 2026-04-26 | — | — | 已补 Java 脚本资源真实 SQL parity：`ScriptResourceRealSqlParityTest` 3 tests 覆盖 derived/filter、join aggregate、union all，SQLite 与 MySQL 5.7 均 3 passed / 0 skipped；包含 `docs/8.2.0.beta/acceptance/` 签收记录；`columnsObj` follow-up：8.3.0.beta P2 已 accepted（见 `docs/8.3.0.beta/acceptance/P2-ComposeQuery-Columns-API-acceptance.md`），Java `columns` API 已收口为单字段 + 单 setter |
