...
| 4 | JaCoCo unit+IT 聚合与关键类门 | ready-for-signoff / formal-r11 evidence-complete | Step 3 exit passed | fresh formal-r11, quality and 35-row audit all pass; the Step 4 canonical spec is `READY_FOR_SIGNOFF`, pending designated-owner decision |
| 5 | authority runner rehearsal / immutable candidate | hold / foggy-projects-signoff-pending | replacement Step 4 signed-off | formal/audit evidence exists but Step 4 is not yet officially accepted; do not start Step 5 before designated signoff issues a decision |
...
- The evidence package is ready, and its Step 4-scoped canonical candidate is now `READY_FOR_SIGNOFF`; the
  designated `foggy-projects` signoff decision is required before official feature acceptance. The separate
  canonical spec remains `ULTRA_EXECUTING` for Steps 5–7. See
