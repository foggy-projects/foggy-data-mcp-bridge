...
当前 committed Redis 与 Mongo/DataViewer subset candidates 分别为 `2/3` 与 `4/30`，合计
关闭 external `6 reports / 33 testcase / F0/E0/S0`；两条 lane 的 12/12 report/manifest
negatives、3/3 candidate-tree local tamper self-checks、6/6 sensitive probes 和 real INT/TERM/HUP
`130/143/129` cleanup 均通过。两个 final manifest 都是 `complete=false`，且来自不同 run，
不能拼接为 full authority，也不能代替 remaining external `10/43`。wrong image/version、
dirty state 和 forced cleanup failure 仍是 Step 3 resource-negative backlog。
...
- 这些 candidates 是 run-local 且 not authority：四外库 fresh replay、16 个 required
  external 中 remaining `10/43`、DB/resource-state negatives 和 portable archive contract
  仍未完成。Committed Redis 与 Mongo subsets 已分别达到 `2/3`、`4/30`，不得与 SQLite
  或彼此跨 run 拼成 full authority。
- evidence：
...
- next executable action: 按既定顺序推进 MCP/MySQL57 `8/23`，再执行 Vector `2/20`，
  同时补 Step 3 DB/resource-state negatives；在 clean
