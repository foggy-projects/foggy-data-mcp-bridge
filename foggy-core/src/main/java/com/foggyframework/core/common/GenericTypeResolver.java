...
public abstract class GenericTypeResolver {
	...
	private static Class[] doResolveTypeArguments(Class ownerClass, Type ifc, Class genericIfc) {
		if (ifc instanceof ParameterizedType paramIfc) {
		...
		if (type.getRawType() instanceof Class) {
			...
			for (int i = 0; i < actualTypeArguments.length; i++) {
				...
				if (actualTypeArgument instanceof Class) {
					...
					typeVariableMap.put(variable, actualTypeArgument);
				} else if (actualTypeArgument instanceof TypeVariable typeVariableArgument) {
	...
	public static Class<?> resolveReturnTypeArgument(Method method, Class<?> genericIfc) {
		...
		if (returnType.equals(genericIfc)) {
			if (genericReturnType instanceof ParameterizedType targetType) {
