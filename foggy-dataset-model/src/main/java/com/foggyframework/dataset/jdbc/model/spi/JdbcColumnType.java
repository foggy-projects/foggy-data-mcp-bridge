...
public enum JdbcColumnType {
    ...
    JdbcColumnType(String code, int jdbcType, Class<?> javaType) {
        this.code = code;
     ...
     */
    public static JdbcColumnType fromCode(String code) {
        if (code == null) {
            return UNKNOWN;
        }
        // 处理常见别名
        ...
        for (JdbcColumnType type : values()) {
     ...
     * @return 枚举值，未找到返回 {@link #UNKNOWN}
     */
    public static JdbcColumnType fromJdbcType(int jdbcType) {
        switch (jdbcType) {
            case Types.INTEGER:
            case Types.SMALLINT:
            case Types.TINYINT:
