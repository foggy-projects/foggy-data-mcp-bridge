...
public class JdbcQueryModelLoaderImpl extends LoaderSupport implements JdbcQueryModelLoader {
     ...
     */
    @Autowired(required = false)
    MongoModelLoader mongoQueryModelLoader;
...
//        RX.notNull(queryModelDef, "需要有查询模型定义");
        ...
        List<JdbcQueryCondition> jdbcQueryConditions = new ArrayList<>();
        if (conds != null) {
            for (QueryConditionDef cond : conds) {
                ...
                JdbcColumn jdbcColumn = qm.findJdbcColumnForCond(column, true);
                ...
                jdbcQueryConditions.add(jdbcQueryCond);
                JdbcDimensionColumn dimensionColumn = jdbcColumn.getDecorate(JdbcDimensionColumn.class);
                if (dimensionColumn != null) {
                    qm.addQueryDimensionIfNotExist(dimensionColumn.getJdbcDimension());
                    jdbcQueryCond.setDimension(dimensionColumn.getJdbcDimension());
                }
                JdbcPropertyColumn jdbcPropertyColumn = jdbcColumn.getDecorate(JdbcPropertyColumn.class);
                if (jdbcPropertyColumn != null) {
                    qm.addQueryPropertyIfNotExist(jdbcPropertyColumn.getJdbcProperty());
                    jdbcQueryCond.setProperty(jdbcPropertyColumn.getJdbcProperty());
                }


            }
        }
        qm.addJdbcQueryConds(jdbcQueryConditions);
        /**
         * step30.为JdbcQueryColumn补jdbcQueryCond
         */
        for (JdbcQueryColumn jdbcQueryColumn : qm.getJdbcQueryColumns()) {
            String condColumnName = jdbcQueryColumn.getName();
            JdbcColumn jdbcColumn = jdbcQueryColumn.getSelectColumn();
            JdbcDimensionColumn dimensionColumn = jdbcColumn.getDecorate(JdbcDimensionColumn.class);
            if (dimensionColumn != null && dimensionColumn.isCaptionColumn()) {
                ...
                JdbcQueryCondition jdbcQueryCondition = qm.findJdbcQueryCondByName(condColumnName);
                if (jdbcQueryCondition == null) {
                    condColumnName = dimensionColumn.getJdbcDimension().getForeignKeyJdbcColumn().getName();
                }

            }
            if (jdbcColumn.isDimension()) {
                //把该维度加到列表
                qm.addQueryDimensionIfNotExist(jdbcColumn.getDecorate(JdbcDimensionColumn.class).getJdbcDimension());
            } else if (jdbcColumn.isProperty()) {
                //把该维度加到列表
                qm.addQueryPropertyIfNotExist(jdbcColumn.getDecorate(JdbcPropertyColumn.class).getJdbcProperty());
            }
            ...
            if (jdbcQueryCondition == null) {
                ...
                jdbcQueryCondition = autoCreateJdbcQueryCond(qm, jdbcQueryColumn, qm.findJdbcColumnForCond(condColumnName, true));
                qm.addJdbcQueryCond(jdbcQueryCondition);
            }
//            jdbcQueryColumn.getDecorate(JdbcQueryColumnImpl.class).setd(jdbcQueryCond);
            jdbcQueryColumn.getDecorate(JdbcQueryColumnImpl.class).setJdbcQueryCondition(jdbcQueryCondition);
        }
        /**
         * step35.加载orders
         */
         ...
         * step50.补一些默认值
         */
        for (JdbcQueryCondition jdbcQueryCondition : qm.getJdbcQueryConditions()) {
            fixJdbcQueryCond(qm, (JdbcQueryConditionImpl) jdbcQueryCondition, qm.findJdbcColumnForCond(jdbcQueryCondition.getName(), false));
        }

        /**
         * 呃，如果存在ID列，默认用它来排
         */
        JdbcQueryColumn idQueryColumn = qm.getIdJdbcQueryColumn();
...
//        qm.setQueryDimensions(qm.getJdbcModel().getDimensions().stream().map(e -> new JdbcQueryDimensionImpl(e)).collect(Collectors.toList()));
        ...
        if (queryModelDef.getColumnGroups() != null && !queryModelDef.getColumnGroups().isEmpty()) {
            ...
            for (JdbcColumnGroupDef columnGroupDef : queryModelDef.getColumnGroups()) {
                ...
                for (SelectColumnDef item : columnGroupDef.getItems()) {
                    ...
                    JdbcDimension dimension = qm.findDimension(ref);
                    if (dimension != null) {
    ...
    private void addColumn(JdbcQueryModelImpl qm, JdbcColumnGroup group, String name, SelectColumnDef item, boolean hasRef) {
        ...
        JdbcQueryColumn jdbcQueryColumn = new JdbcQueryColumnImpl(jdbcColumn, item.getName(), item.getCaption(), item.getAlias(), item.getField());
        jdbcQueryColumn.setHasRef(hasRef);
        jdbcQueryColumn.getDecorate(JdbcQueryColumnImpl.class).setUi(item.getUi());

        qm.addJdbcQueryColumn(jdbcQueryColumn);
    ...
    private void fixJdbcQueryCond(JdbcQueryModelImpl qm, JdbcQueryConditionImpl jdbcQueryCond, JdbcColumn selectColumn) {
        ...
        JdbcDimensionColumn jdbcDimensionColumn = selectColumn.getDecorate(JdbcDimensionColumn.class);
        JdbcMeasureColumn jdbcMeasureColumn = selectColumn.getDecorate(JdbcMeasureColumn.class);
        JdbcPropertyColumn jdbcPropertyColumn = selectColumn.getDecorate(JdbcPropertyColumn.class);
        String autoQueryType = "=";
        JdbcQueryCondType autoType = null;

        if (jdbcDimensionColumn != null && jdbcPropertyColumn == null) {
            //仅当jdbcQueryCond中的jdbcColumn为foreignKey列时，才使用下拉查询

            JdbcDimension dimension = jdbcDimensionColumn.getJdbcDimension();
            QueryObject dimQueryObject = dimension.getQueryObject();
            jdbcQueryCond.setDimension(dimension);

            if (dimQueryObject != null && !jdbcDimensionColumn.isCaptionColumn()) {
                //有关联的维表,且不是caption列
                jdbcQueryCond.setType(JdbcQueryCondType.DIM);

            } else {
                JdbcDimensionType dimType = dimension.getType();
                Tuple2<String, JdbcQueryCondType> r = autoFix(jdbcQueryCond, dimType);
            ...
            autoQueryType = "[]";
        } else if (jdbcPropertyColumn != null) {
            jdbcQueryCond.setProperty(jdbcPropertyColumn.getJdbcProperty());
            JdbcDimensionType dimType = JdbcDimensionType.fromColumnType(jdbcPropertyColumn.getJdbcProperty().getType());
            Tuple2<String, JdbcQueryCondType> r = autoFix(jdbcQueryCond, dimType);
            autoQueryType = r.getT1();
            autoType = r.getT2();
        }
    ...
    private Tuple2<String, JdbcQueryCondType> autoFix(JdbcQueryConditionImpl jdbcQueryCond, JdbcDimensionType type) {
        String autoQueryType = "=";
        JdbcQueryCondType autoType = null;
        if (type == JdbcDimensionType.DATETIME) {
            //日期维
            autoQueryType = "[)";
            autoType = JdbcQueryCondType.DATE_RANGE;
        } else if (type == JdbcDimensionType.DICT) {
            //字典表维
            autoType = JdbcQueryCondType.DICT;
        } else if (type == JdbcDimensionType.BOOL) {
            //boolean
            autoType = JdbcQueryCondType.BOOL;
        } else if (type == JdbcDimensionType.DOUBLE) {
            //boolean
            autoType = JdbcQueryCondType.DOUBLE;
        } else if (type == JdbcDimensionType.INTEGER) {
            //boolean
            autoType = JdbcQueryCondType.INTEGER;
        } else if (type == JdbcDimensionType.DAY) {
            //字典表维
            autoQueryType = "[]";
            autoType = JdbcQueryCondType.DAY_RANGE;
        } else {
            jdbcQueryCond.setType(JdbcQueryCondType.COMMON);
            autoType = JdbcQueryCondType.COMMON;
        }
    ...
    private JdbcQueryConditionImpl autoCreateJdbcQueryCond(JdbcQueryModelImpl qm, JdbcQueryColumn jdbcQueryColumn, JdbcColumn selectColumn) {
        ...
        if (jdbcQueryColumn.isHasRef()) {
            jdbcQueryCond.setField(jdbcQueryColumn.getField());
            jdbcQueryCond.setName(jdbcQueryColumn.getName());
        } else {
...
//                RX.hasText(accessDef.getDimension(), "access必须定义维度");
                ...
                JdbcQueryDimension jdbcDimension = qm.findQueryDimension(dimension, false);
                ...
                JdbcQueryAccessImpl impl = new JdbcQueryAccessImpl();
                BeanUtils.copyProperties(accessDef, impl);
                jdbcDimension.getDecorate(JdbcQueryDimensionImpl.class).setQueryAccess(impl);
            } else if (StringUtils.isNotEmpty(property)) {
                JdbcQueryProperty jdbcQueryProperty = qm.findQueryProperty(property, false);
                if (jdbcQueryProperty == null) {
                    //把该维度加到列表
                    jdbcQueryProperty = qm.addQueryPropertyIfNotExist(qm.findProperty(property, true));
                }

                RX.notNull(jdbcQueryProperty, "未能找到维度" + dimension);
                JdbcQueryAccessImpl impl = new JdbcQueryAccessImpl();
                BeanUtils.copyProperties(accessDef, impl);
                jdbcQueryProperty.getDecorate(JdbcQueryPropertyImpl.class).setQueryAccess(impl);
