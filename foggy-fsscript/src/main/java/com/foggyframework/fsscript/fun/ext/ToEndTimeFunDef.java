...
public final class ToEndTimeFunDef extends AbstractFunDef implements FunDef {
    ...
    public Object execute(ExpEvaluator evaluator, Exp[] args) {
        ...
        if (str instanceof Date) {
            return DateUtils.toEndTime((Date) str);
        }
        Date d = null;
        if (str.toString().length() > 14) {
            d = ObjectTransFormatter.DATE_TRANSFORMATTERINSTANCE.format(str.toString());
            d = DateUtils.toEndTime(d);
        } else {
            d = ObjectTransFormatter.DATE_TRANSFORMATTERINSTANCE.format(str);
            d = DateUtils.toEndTime(d);
        }
