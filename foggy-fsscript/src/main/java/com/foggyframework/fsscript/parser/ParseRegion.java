/*
// $Id: ParseRegion.java 315 2010-05-29 00:56:11Z jhyde $
 ...
 *
 * <p>
 * The main purpose of a ParseRegion is to give detailed locations in error
 * messages and warnings from the parsing and validation process.
 *
 * <p>
 * A region has a start and end line number and column number. A region is a
 * point if the start and end positions are the same.
 *
 * <p>
 ...
 *
 * @version $Id: ParseRegion.java 315 2010-05-29 00:56:11Z jhyde $
 * @author jhyde
 */
public class ParseRegion implements Serializable {
	private static final long serialVersionUID = 2408729041181367732L;

	/**
	 * Combination of a region within an MDX statement with the source text of
	 * the whole MDX statement.
	 *
	 * <p>
	 * Useful for reporting errors. For example, the error in the statement
	 *
	 * <blockquote>tpl_CD_express_order_admin.fpage
	 * 
	 * <pre>
	 ...
	 * region [1:9, 1:34].
	 */
	public static class RegionAndSource implements Serializable{
		private static final long serialVersionUID = -5415584904252476515L;
	 ...
	 * @param endCol
	 *            Column number of end of region
	...
	public static RegionAndSource findPos(String code) {
		...
		if (secondCaret < 0) {
			...
			String codeSansCaret = code.substring(0, firstCaret) + code.substring(firstCaret + 1, secondCaret)
	...
	private static ParseRegion sum(Iterable<ParseRegion> regions, int startLine, int startColumn, int endLine,
			int endColumn) {
		int testLine;
		int testColumn;
	 ...
	 *
	 * @param line
	 *            Line of the beginning and end of the region
	 * @param column
	 ...
	 *
	 * <p>
	 * All lines and columns are 1-based and inclusive. For example, the token
	 * "select" in "select from [Sales]" has a region [1:1, 1:6].
	 *
	 * @param startLine
	 *            Line of the beginning of the region
	 * @param startColumn
	 *            Column of the beginning of the region
	 * @param endLine
	 *            Line of the end of the region
	 * @param endColumn
	 ...
	 *
	 * <p>
	 * For example, for the region <code>(1, 9, 1, 12)</code> and source
	 * <code>"values (foo)"</code>, yields the string
	...
	public boolean equals(Object obj) {
		if (obj instanceof ParseRegion) {
			...
			return this.startLine == that.startLine && this.startColumn == that.startColumn
					&& this.endLine == that.endLine && this.endColumn == that.endColumn;
		} else {
			return false;
		}
	}
	...
	@Override
	public int hashCode() {
		return startLine ^ (startColumn << 2) ^ (endLine << 4) ^ (endColumn << 8);
	}
	...
	/**
	 * Combines this region with a list of parse tree nodes to create a region
	 * which spans from the first point in the first to the last point in the
	 * other.
	 *
	 * @param regions
	...
	public ParseRegion plusAll(Iterable<ParseRegion> regions) {
		return sum(regions, getStartLine(), getStartColumn(), getEndLine(), getEndColumn());
	}
	...
	public String toString() {
		return "[" + startLine + ":" + startColumn + ((isPoint()) ? "" : ", " + endLine + ":" + endColumn) + "]";
	}
}
