package com.foggyframework.dataset.model.impl.dimension;
 ...
 *       行为与普通维度相同，精确匹配</li>
 *   <li><b>层级视角</b>（team$hierarchy$id, team$hierarchy$caption）：使用闭包表，
 *       在 slice 中匹配该节点及所有后代，在 columns/groupBy 中汇总到祖先节点</li>
 * </ul>
 *
 * <p>查询示例：
 * <ul>
 *   <li>精确匹配：slice team$id=T001 → 只查 T001 自身</li>
 *   <li>层级汇总：slice team$hierarchy$id=T001, groupBy team$hierarchy$caption → 返回 1 条（汇总到 T001）</li>
 *   <li>后代明细：slice team$hierarchy$id=T001, groupBy team$caption → 返回 N 条（各后代明细）</li>
 * </ul>
 */
@Data
@NoArgsConstructor
public class DbModelParentChildDimensionImpl extends DbDimensionSupport {
    ...
    /**
     * 层级视角的主键列（team$hierarchy$id）
     */
    DbColumn hierarchyPrimaryKeyDbColumn;

    /**
     * 层级视角的标题列（team$hierarchy$caption）
     */
    DbColumn hierarchyCaptionDbColumn;

    /**
     * 层级视角的属性列列表（team$hierarchy$xxx）
     */
