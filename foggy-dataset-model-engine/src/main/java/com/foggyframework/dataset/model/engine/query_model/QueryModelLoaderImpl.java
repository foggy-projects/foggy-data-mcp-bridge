...
import com.foggyframework.dataset.model.def.query.QueryConditionDef;
...
import com.foggyframework.dataset.model.proxy.AggregateRelationProxy;
import com.foggyframework.dataset.model.proxy.ColumnRef;
import com.foggyframework.dataset.model.proxy.DimensionProxy;
import com.foggyframework.dataset.model.proxy.JoinBuilder;
...
import com.foggyframework.dataset.model.semantic.member.SyntheticMemberQueryModelFactory;
...
public class QueryModelLoaderImpl extends LoaderSupport implements QueryModelLoader {
    ...
    public QueryModelLoaderImpl(TableModelLoaderManager tableModelLoaderManager,
        ...
        this(tableModelLoaderManager, systemBundlesContext, fileFsscriptLoader, queryModelBuilders,
                tableModelLoaderManager instanceof com.foggyframework.dataset.model.impl.loader.TableModelLoaderManagerImpl manager
            ...
            try {
                ...
                if (prepared.synthetic()) {
                    ...
                    stageQueryModel(
                            ...
                            sourceOf(candidate, prepared.sourceModelName()));
                } else {
                    ...
                    stageQueryModel(
                            ...
                            false,
                            Set.of(),
                            sourceOf(prepared.fsscript(), prepared.namespace()));
                }
    ...
    public QueryModel stageForRefresh(
        ...
        try {
            ...
            stageQueryModel(
                    ...
                    false,
                    Set.of(),
                    sourceOf(fsscript, canonicalNamespace));
            return built;
        } catch (Throwable failure) {
                ...
                if (tableModel != null && tableModel.getName() != null && !tableModel.getName().isBlank()) {
                    ...
                    if (!synthetic) {
                        ...
                        boolean externalManager = !(tableModelLoaderManager instanceof
                                com.foggyframework.dataset.model.impl.loader.TableModelLoaderManagerImpl);
    ...
    private ModelProvenance.ModelSource sourceOf(
            CatalogCandidate candidate,
            String sourceModelName
    ) {
        ModelProvenance provenance = candidate.modelProvenance(
                CatalogModelKey.query(sourceModelName));
        if (provenance == null) {
            provenance = candidate.modelProvenance(
                    CatalogModelKey.syntheticQuery(sourceModelName));
        }
        return provenance == null ? null : provenance.source();
    }
    ...
    private ModelProvenance.ModelSource sourceOf(
        ...
        if (fsscript == null
                || fsscript.getFsscriptClosureDefinition() == null
                || fsscript.getFsscriptClosureDefinition()
                .getFsscriptClosureDefinitionSpace() == null) {
            return null;
        }
        Bundle bundle = fsscript.getFsscriptClosureDefinition()
                ...
                .getBundle();
        if (bundle == null
                || StringUtils.isEmpty(bundle.getName())
                || StringUtils.isEmpty(fsscript.getPath())) {
            return null;
        }
    ...
    private ExpEvaluator evalQmScript(Fsscript fsscript) {
        ...
        ee.getCurrentFsscriptClosure().setVar("loadTableModel",
                com.foggyframework.dataset.model.proxy.LoadTableModelFunction.getInstance());
        fsscript.eval(ee);
        return ee;
    }
