...
import com.foggyframework.dataset.model.semantic.util.QueryErrorSanitizer;
import com.foggyframework.dataset.model.semantic.util.SanitizedQueryExecutionException;
...
public class JdbcQueryModelImpl extends QueryModelSupport implements JdbcQueryModel {
    ...
    public SqlGenerationResult generateSql(SystemBundlesContext systemBundlesContext, ModelResultContext context) {
        ...
        return queryEngine.toSqlGenerationResult(diagnostics);
    }
    ...
    public com.foggyframework.dataset.model.plugins.query_execution.ManagedSqlRelation prepareManagedRelation(
        ...
        if (queryExecutionStepExecutor != null && queryExecutionStepExecutor.hasSteps()) {
            queryExecutionStepExecutor.executeBeforeExecute(com.foggyframework.dataset.model.plugins.query_execution.QueryExecutionPhase.PREPARE_MANAGED_RELATION, execCtx);
        }

        // ===== Dialect capability validation =====
        FDialect dialect = getDialect();
        if (options != null && options.getRequiredDialectCapabilities() != null) {
            for (com.foggyframework.dataset.model.plugins.query_execution.ManagedRelationOptions.DialectCapability cap
        ...
        java.util.List<com.foggyframework.dataset.model.plugins.query_execution.ManagedMetricMetadata> metricMetadataList =
                buildMetricMetadata(context);

        // 注意：prepare 阶段不短路执行，即使缓存命中也会继续，除非外层决定使用它。
        return new com.foggyframework.dataset.model.plugins.query_execution.ManagedSqlRelation(
                execCtx.getSql(),
                execCtx.getParams(),
                dialect,
                queryEngine,
     ...
     */
    private java.util.List<com.foggyframework.dataset.model.plugins.query_execution.ManagedMetricMetadata> buildMetricMetadata(
            ModelResultContext context) {
        java.util.List<com.foggyframework.dataset.model.plugins.query_execution.ManagedMetricMetadata> result = new java.util.ArrayList<>();
        ...
        for (TableModel tm : getJdbcModelList()) {
            ...
            for (DbMeasure measure : tm.getMeasures()) {
                ...
                com.foggyframework.dataset.model.plugins.query_execution.AdditiveKind kind;
                String aggFunc;
                if (agg == null || agg == DbAggregation.NONE || agg == DbAggregation.PK) {
                    kind = com.foggyframework.dataset.model.plugins.query_execution.AdditiveKind.UNKNOWN;
                    aggFunc = "SUM";
                } else {
                    switch (agg) {
                        case SUM:
                        ...
                        case MIN:
                        case MAX:
                            kind = com.foggyframework.dataset.model.plugins.query_execution.AdditiveKind.ADDITIVE;
                            break;
                        case AVG:
                        case COUNT_DISTINCT:
                            kind = com.foggyframework.dataset.model.plugins.query_execution.AdditiveKind.NON_ADDITIVE;
                            break;
                        default:
                            kind = com.foggyframework.dataset.model.plugins.query_execution.AdditiveKind.UNKNOWN;
                            break;
                ...
                result.add(com.foggyframework.dataset.model.plugins.query_execution.ManagedMetricMetadata.builder()
    ...
    public DbQueryResult executeManagedRelation(QueryExecutionContext executionContext, String finalSql, List<Object> finalParams) {
        ...
        if (queryExecutionStepExecutor != null && queryExecutionStepExecutor.hasSteps()) {
            queryExecutionStepExecutor.executeAfterExecute(com.foggyframework.dataset.model.plugins.query_execution.QueryExecutionPhase.EXECUTE_MANAGED_RELATION, executionContext);
        }
