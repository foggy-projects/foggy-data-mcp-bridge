...
class PreAggregationDataValidationTest {
    ...
    @Test
    @Order(10)
    @DisplayName("故意修改预聚合数据后，通过AdvancedQueryFacade检测到预聚合与原始表查询结果差异")
    void testDetectCorruptedPreAggData() {
        ...
        try {
            ...
            // 使用 AdvancedQueryFacade 执行查询（启用预聚合）
            ...
            // 使用 AdvancedQueryFacade 执行查询（禁用预聚合）
            ...
            log.info("AdvancedQueryFacade 查询结果: 预聚合查询总额={}, 原始表查询总额={}",
            ...
            log.info("AdvancedQueryFacade 错误检测验证通过: 预聚合查询={}, 原始查询={}, 差异={}",
    ...
    @Test
    @Order(20)
    @DisplayName("验证 preAggEnabled 开关正常工作（通过 AdvancedQueryFacade）")
    void testPreAggEnabledSwitch() {
        ...
        // 测试1: preAggEnabled = true（通过 AdvancedQueryFacade）
        ModelResultContext contextWithPreAgg = new ModelResultContext(pagingRequest, null);
        contextWithPreAgg.setCacheConfig(ModelResultContext.QueryCacheConfig.builder()
                .l1Enabled(false)
        ...
        // 测试2: preAggEnabled = false（通过 AdvancedQueryFacade）
        ModelResultContext contextNoPreAgg = new ModelResultContext(pagingRequest, null);
        contextNoPreAgg.setCacheConfig(ModelResultContext.QueryCacheConfig.builder()
                .l1Enabled(false)
                .l2Enabled(false)
    ...
    private ComparisonResult executeComparisonQuery(String queryModelName,
        ...
        // 查询1: 使用预聚合（通过 AdvancedQueryFacade）
        ...
        // 查询2: 不使用预聚合（通过 AdvancedQueryFacade）
        ModelResultContext contextNoPreAgg = new ModelResultContext(pagingRequest, null);
        contextNoPreAgg.setCacheConfig(ModelResultContext.QueryCacheConfig.builder()
                .l1Enabled(false)
                .l2Enabled(false)
