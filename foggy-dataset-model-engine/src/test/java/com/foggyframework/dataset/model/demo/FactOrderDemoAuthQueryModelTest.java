 ...
 * 订单查询模型（完整权限演示）单元测试
 *
 * <p>测试 FactOrderDemoAuthQueryModel 的权限控制功能：
 * <ul>
 *   <li>Spring Bean 注入（@demoAuthorizationService）</li>
...
@DisplayName("订单查询模型（完整权限演示）单元测试")
class FactOrderDemoAuthQueryModelTest extends DemoTestSupport {
    ...
    void testAccessControlSqlGeneration() {
        JdbcQueryModel queryModel = getQueryModel("FactOrderDemoAuthQueryModel");
    ...
    void testAccessControlDataComparison() {
        ...
        Long totalCount = getTableCount("fact_order");
        log.info("fact_order 总记录数: {}", totalCount);
    ...
    void testCompareWithoutAccess() {
        ...
        JdbcQueryModel noAccessModel = getQueryModel("FactOrderQueryModel");
        ...
        JdbcQueryModel withAccessModel = getQueryModel("FactOrderDemoAuthQueryModel");
    ...
    void testAccessWithUserSlice() {
        ...
        com.foggyframework.dataset.model.def.query.request.SliceRequestDef slice =
                new com.foggyframework.dataset.model.def.query.request.SliceRequestDef();
        slice.setField("amount");
        slice.setOp(">=");
        slice.setValue(500);
        ...
        boolean hasUserFilter = values.stream()
                .anyMatch(v -> v instanceof Number && ((Number) v).doubleValue() == 500.0);
        assertTrue(hasUserFilter, "参数中应包含用户过滤值 500");
    ...
    void testAliasResolution() {
        ...
        assertTrue(sql.contains("t0") || sql.contains("fact_order"),
                "SQL应包含主表");
        assertTrue(sql.contains("dim_customer") || sql.toLowerCase().contains("join"),
                "SQL应包含维度表JOIN");
