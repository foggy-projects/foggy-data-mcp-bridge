# FSScript SQL 辅助函数使用指南

[English](FSScript-SQL-Functions.md)

本文档介绍 Foggy Dataset 提供的 SQL 辅助函数,用于在 FSScript 中安全地构建动态 SQL 查询。
...
**参数**:
- `value` - 参数值,可以是任意类型
- `sqlFragment` - SQL 片段,使用 `?` 作为占位符
- `force` (可选) - 是否强制添加,即使 value 为空(默认 false)
...
**参数**:
- `array` - 数组或集合
- `sqlPrefix` - SQL 前缀,如 "AND status IN "
- `force` (可选) - 是否强制添加(默认 false)
